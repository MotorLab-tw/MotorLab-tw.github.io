...
MANUAL = {
    ...
    "i18n": {
        "zh": {
            ...
            "description": "MotorLab Mini 4WD® 馬達磨合機完整使用手冊 — 連線、馬達特性量測、磨合兩種模式、AI 智慧磨合、馬達測試、歷史紀錄、全球磨合資料庫、AI 健康管理、軸承/電刷測試、田宮馬達規格、系統設定、安全保護、WiFi 重設與常見問題。對應韌體 v3.7.0。",
            "keywords": "MotorLab 使用手冊, 馬達磨合機操作, 馬達磨合機說明書, MotorLab 教學, 馬達磨合機設定",
            "breadcrumb": "使用者手冊",
            "eyebrow": "Docs · 使用者手冊",
            "h1": "MotorLab 使用者手冊",
            "lead": "Mini 4WD® 馬達磨合機完整操作說明。對應韌體 v3.7.0。功能隨版本更新,新版可能多出未列選項。",
            "toc_label": "目錄",
            "fw_note": "本手冊適用 MotorLab v3.7.0 韌體(2026-08-08)。",
        },
        "en": {
            "title": "MotorLab User Manual: Motor Break-in Machine Guide | MotorLab",
            "description": "Complete user manual for the MotorLab Mini 4WD® motor break-in machine — connection, motor characterization, two-mode break-in, AI smart break-in, testing, history records, global database, AI health management, bearing/brush tests, Tamiya motor specs, system settings, safety, WiFi reset and FAQ. For firmware v3.7.0.",
            ...
            "lead": "Complete operating guide for the Mini 4WD® motor break-in machine. For firmware v3.7.0. Features evolve with each release; newer firmware may add options not listed here.",
            "toc_label": "Contents",
            "fw_note": "This manual applies to MotorLab firmware v3.7.0 (2026-08-08).",
        },
        "ja": {
            "title": "MotorLab ユーザーマニュアル:モーター慣らし機操作ガイド | MotorLab",
            "description": "MotorLab Mini 4WD® モーター慣らし機の完全ユーザーマニュアル — 接続、モーター特性測定、慣らし 2 モード、AI スマート慣らし、テスト、履歴記録、グローバルデータ庫、AI 健康管理、ベアリング/ブラシ測定、タミヤモーター規格、システム設定、安全保護、WiFi リセット、FAQ。ファームウェア v3.7.0 対応。",
            ...
            "lead": "Mini 4WD® モーター慣らし機の完全操作ガイド。ファームウェア v3.7.0 対応。機能はバージョンごとに更新され、新版では未記載の項目が増える場合があります。",
            "toc_label": "目次",
            "fw_note": "本マニュアルは MotorLab ファームウェア v3.7.0（2026-08-08）に対応します。",
        },
    },
    "sections": {
        "zh": [
            ...
            {"id": "charexp", "t": "四、馬達特性量測", "html": (
                "<p class='manual-intro'>全自動量出馬達的特性參數與損耗,<b>給參考數據、不評分不排名</b>——同型號馬達互相比較、自己判斷。系統自動<b>連續量測 3 趟取平均</b>,約 5 分鐘,過程溫和,適合全新未磨合馬達。</p><p><b>操作</b>:首頁 →「馬達特性量測」→ 選方向(預設正轉)+ 選<b>馬達型號</b>(16 款下拉)+ 填<b>備註</b>(選填,40 字以內)→「開始量測」→ 全程自動(顯示第幾趟與進度),過程中請勿觸碰馬達 → 完成顯示數據表並<b>自動存入量測紀錄</b>。</p><p><b>怎麼看數據(表內每項都標了方向)</b>:</p><ul><li><b>★ I0(無載電流)</b>:同轉速下<b>越低越好</b>——重複性最佳,選別馬達的首要指標。</li><li><b>★ Km(品質因數)/ ★ T_loss(損耗轉矩)</b>:輔助指標(Km 越高越好、T_loss 越低越好)。</li><li><b>Ke(磁路強度)</b>:無絕對好壞——數值相近的馬達互相比較才準確(偏高=偏扭力、偏低=偏轉速)。</li><li><b>KV(轉速常數,rpm/V)</b>:由 Ke 換算——KV × 電壓 ≈ 空載轉速(例 KV 14000 × 3V ≈ 42000 RPM),方便與市面標示對照。</li><li><b>R(內部電阻)</b>:越低越好。</li><li><b>起轉電壓</b>:僅參考——單次隨機性大,適合當同一顆馬達的長期健康對照(日後明顯升高=軸承 / 電刷劣化警訊)。</li><li><b>損耗擬合 R² / Ke 漂移</b>:資料品質指標(越接近 1、漂移越低越可信)。</li><li><b>3 趟一致性</b>:各趟 I0 的最大差異;顯示 ⚠ 代表馬達不穩或架設鬆動,該筆數據僅供參考,建議檢查後重測。</li></ul><p><b>量測紀錄</b>:量測頁 →「量測紀錄」。每次成功量測自動儲存(含全部指標與各轉速點資料),最多 <b>50 筆</b>,滿了自動刪最舊;可檢視詳情、可刪除。</p><p><b>建議用法</b>:</p><ul><li>同批新馬達逐顆量測 → 依 ★I0 挑體質好的再投入磨合(省磨合工時)。</li><li>磨合後<b>再測同一顆</b>對照:損耗類應下降(磨合有效);Ke / R 應幾乎不變。</li><li>比較不同馬達盡量在<b>相近溫度</b>下測;馬達剛裝上或冷機重啟後第一次數據可能偏高,以第二次起為準。</li></ul><div class='manual-note'><b>統一量測基準溫度(v3.7.0 起,全自動)</b>:溫度會影響量測——系統在每次量測前自動確認機器達到<b>量測基準溫度</b>,未達就先自動暖機(最多約 3 分鐘,顯示「暖機中」)再開始,讓每次量測都在同一溫度下進行(AI 扭力預測、健康管理同樣受此保護);環境過低／過高時提示「數據僅供參考」,測試照常。此外<b>磁力類數值(Ke／Km／KV)已自動換算為 25°C 基準值</b>,不同天、不同季節量的結果可直接比較。</div><div class='manual-note'><b>安全保護(全程自動)</b>:機器在任何驅動馬達的程序(磨合／測試／特性量測／軸承／電刷／健康檢測)中全程自動監測,異常時立即停止並跳出說明框——未偵測到馬達(< 3 秒)、馬達卡住(< 5 秒)、讀不到轉速訊號(< 4 秒)、輸出短路(緊急停止 < 0.15 秒)、高溫(立即停 + 鎖定)。觸發時蜂鳴器響 4 長聲。<b>供電規格 DC 5V／6A 不可低於</b>;供電不足時頂部出現琥珀色警示「可能無法達到設定電壓」。</div><div class='manual-note'><b>注意</b>:量測失敗(馬達未起轉 / 鎖定逾時 / 過流)會顯示原因,重試即可。若反覆失敗或待機時轉速不為 0,請檢查轉速感測器與反射盤的對位。</div>"
            )},
            {"id": "breakin", "t": "五、馬達磨合(兩種模式)", "html": (
                "<p class='manual-intro'>低速長時間運轉,讓電刷與整流子貼合到最佳接觸。磨合頁最上方有<b>兩個模式按鈕</b>(運轉中不可切換)。</p><table class='manual-table'><thead><tr><th>模式</th><th>適合</th><th>一句話</th></tr></thead><tbody><tr><td><b>電壓模式</b></td><td>有自己配方的玩家</td><td>各階段固定電壓運轉(傳統方式)</td></tr><tr><td><b>轉速模式 (Beta)</b></td><td>想以轉速為基準的玩家</td><td>各階段自動控速在設定轉速,電壓穩定即自動換階</td></tr></tbody></table><div class='manual-note'>想「一鍵全自動、磨好自動停」的玩家,請用首頁的獨立功能「<b>AI 智慧磨合</b>」(Pro,詳見「六、AI 智慧磨合」)。</div><h3>電壓模式</h3><p>首頁 →「馬達磨合程式」→ 選<b>馬達型號</b>(16 款)+ 填<b>備註</b>(40 字以內)→ 選<b>判停標準</b>(純時間限制=各階段跑滿設定時間;智慧穩定=電流提前穩定就換階,時間為上限)→ 選<b>磨合趟數</b>(1~6,預設 1)→ 逐趟確認 10 階段參數(多趟用分頁籤切換趟;直接點數字欄修改)→「啟動」。</p><ul><li>每階段可設:電壓(0.6~4.0V,<b>0=跳過</b>)/ 方向 / 磨合秒(10~600,預設 60)/ 冷卻秒(10~600,預設 60)/ 穩定電流容差(2~50mA)。</li><li>多趟可按「<b>本趟複製到之後所有趟</b>」快速填表;趟數旁即時顯示<b>預估總時長</b>。</li><li>輸入超出範圍會自動改為上下限並閃光提示。</li></ul><div class='manual-note'><b>重點</b>:使用新馬達磨合時,碳刷及整流子尚未完成整形,可依趟數將容差值的設定逐漸由大縮小。</div><h3>轉速模式 (Beta)</h3><p>與電壓模式相同的階段表,但第一欄改為<b>目標轉速</b>(6000~30000,每 1000 一階,<b>0=跳過</b>)——機器自動調整電壓把馬達鎖定在該轉速。</p><ul><li><b>判停標準</b>(嚴格 / 標準 / 寬鬆):馬達在該轉速下驅動電壓穩定即視為磨順、提前換階;磨合秒為上限。嚴格=要更穩才過關(磨得較透);寬鬆=較快換階(全新馬達一直換不了階可改寬鬆)。</li><li>階段結果欄顯示<b>穩定電壓</b>;推不到目標轉速會以安全上限電壓跑滿時間並標「<b>未達標</b>」(通常代表目標轉速訂太高,或馬達 / 治具異常)。</li><li>方向、時間、趟數、複製等操作皆與電壓模式相同。</li></ul><h3>共同行為</h3><p>每階段流程:緩啟動 → 運轉中 → 緩停止 → 冷卻中 → 下一階段;跑完一趟自動接下一趟(顯示「第 x/y 趟 · 階段 c」)。全部完成 → 嗶 3 聲 → 自動存入歷史紀錄(電壓 / 轉速模式,含每趟每階段結果)。</p><p><b>運轉中可按</b>:</p><table class='manual-table'><thead><tr><th>按鈕</th><th>行為</th></tr></thead><tbody><tr><td>暫停運轉</td><td>緩停後<b>暫停</b>,按鈕變為「<b>恢復運轉</b>」;再按即從<b>斷點接續</b>——階段已跑的秒數保留不重算、冷卻倒數接續。暫停無時限;<b>暫停期間其餘按鈕全部鎖定,只有「恢復運轉」可按</b></td></tr><tr><td>停止</td><td>緩停後結束,紀錄標「使用者中止」</td></tr><tr><td>回首頁</td><td>切回首頁,磨合<b>背景繼續跑</b></td></tr></tbody></table><div class='manual-note'><b>關於暫停</b>:暫停期間馬達停轉降溫,恢復續磨等同在該階段插入一次額外冷卻,對磨合無害;恢復後該階段的「穩定判定」會重新收集(避免冷熱狀態混判)。<b>斷電不保留暫停進度</b>——中途斷電視同中止。(「歸零」須待機時才可按,清除上一次運轉的顯示紀錄。)</div><div class='manual-note'><b>注意</b>:運轉中所有設定鎖定(暫停中亦同);勿中途斷電(資料遺失);高溫會自動停止並存檔;想快速驗機就把每階段時間改短。即時數據多一格 <b>KV(rpm/V)</b>＝目前轉速除以馬達電壓,供快速對比馬達狀態;另有「<b>本階段運轉 x / y 秒</b>」直接顯示本階段的磨合進度——<b>只累計全速運轉的秒數</b>(緩啟動／緩停止／暫停都不計入),到 y 秒即換階。</div><div class='manual-note'><b>重要:測量／磨合請勿使用「導電潤滑油」。</b>市面標榜「導電」的磨合油(含導電碳粉)加入後轉速瞬升、電壓瞬降,那<b>不是真的磨好,而是碳粉暫時導電的假象</b>。本機所有判定(穩定電流 / 轉速 / 內阻)量測的是電刷與整流子的<b>真實接觸狀態</b>,導電油會使讀數失真偏樂觀(這類油品說明書自己也註明「洗淨後才穩定、才會更好轉」),油一洗掉就打回原形。正確做法:全程不用這類油;若已使用,請徹底洗淨後再上機量測。一般軸承微量潤滑不受影響,此警告僅針對「導電型磨合油」。</div>"
            )},
            {"id": "smartbreakin", "t": "六、AI 智慧磨合(Pro)", "html": (
                "<p class='manual-intro'>一鍵全自動磨合:系統自己磨、自己量、磨好自動停;內建<b>磨合導引</b>跨場判讀,回答磨合最難的問題——「<b>這顆馬達到底要磨幾場才夠?</b>」</p><p><b>操作</b>:首頁 →「AI 智慧磨合」→ 選<b>馬達型號</b>(田宮全 15 款)＋填<b>備註</b>(選填,40 字以內)＋選<b>時間上限</b>(磨到完成為止／2h／3h／4h)→「啟動」,其餘全自動——系統以漸進轉速磨合並持續量測,<b>偵測到穩態(本場不再進步)時自動停止</b>並嗶聲提示。</p><ul><li>頁面上方的即時數據與圖表<b>與磨合頁完全相同</b>;下方顯示:目前輪次／目前動作／已進行時間／收斂進度,及<b>磨合指標曲線</b>(每輪落點＋移動平均,曲線走平＝本場接近完成)。</li><li>過程中馬達會<b>正反轉交替</b>磨合(電刷兩面對稱貼合),並週期性自動量測,聲音與轉速有規律變化屬正常。</li><li><b>等溫取樣</b>:第一輪為<b>熱身基準輪</b>(磨合照常進行、建立本場溫度基準,不列入數據與曲線);第二輪起每輪量測前,系統自動等待機器回到溫度基準再量(必要時最多等數分鐘,<b>期間馬達暫停轉動屬正常</b>)——無論寒帶或熱帶,每輪判定樣本都在同一溫度下取得,磨合判定不受環境氣溫影響。</li><li>按「停止」<b>立即停止</b>,已完成輪次的數據照常保留;手動停止的場次會列入導引歷史但<b>不參與判定</b>(標 ✗)。</li></ul><h3>磨合導引——回答「要磨幾場」</h3><p><b>一場磨完＝馬達磨好了嗎?不一定。</b>自動磨合每一「場」磨到穩態就停,但一顆新馬達通常要<b>連續好幾場</b>才會真正到位;到位之後再磨,不但沒有進步,反而開始消耗電刷與磁鐵壽命(見「二十、物理知識」的「退磁」與「磨合黃金點」條)。磨合導引的任務就是幫你抓住「<b>該停手的那一場</b>」。</p><p><b>使用邏輯(它怎麼運作)</b>:每完成一場,系統自動記下該場的收斂結果;下一場磨完,自動與之前各場比較——數值還在明顯下降＝馬達還在進步;連續持平＝進步吃完了＝<b>磨合黃金點</b>。判定結果直接顯示在面板橫幅:</p><table class='manual-table'><thead><tr><th>橫幅顯示</th><th>意思</th><th>你該做什麼</th></tr></thead><tbody><tr><td>導引觀察中——再磨一場開始判定</td><td>場數還不夠,尚無法判讀</td><td>繼續正常磨</td></tr><tr><td>還在進步,繼續磨划算(較初始已省 x mA)</td><td>這顆還有進步空間</td><td><b>再磨一場划算</b></td></tr><tr><td>已到黃金點,建議停止——再磨進步有限</td><td>進步已吃完,就是現在</td><td><b>停手,這顆收工</b></td></tr><tr><td>已過頭——現在磨的是壽命,建議立即停止</td><td>黃金點已在前幾場出現</td><td>立即停,這顆之後別再磨</td></tr></tbody></table><p><b>使用時機(什麼時候看、什麼時候按)</b>:</p><ol><li><b>新馬達第一場</b>:磨完只會看到「觀察中」——第一場是在建立這顆的基準,屬正常。</li><li><b>之後每場磨完看一眼橫幅</b>:顯示「還在進步」就再開一場;顯示「<b>已到黃金點</b>」就停——這顆的磨合就此完成,直接上車。</li><li><b>啟動時的「導引紀錄」提問</b>(有紀錄時才出現):接著磨<b>同一顆</b>→選「同一顆繼續」(預設);已換上<b>另一顆</b>馬達→選「<b>新馬達(重新開始)</b>」,導引紀錄歸零重新累積——不歸零會拿別顆的紀錄亂比,判定就失準。</li><li><b>忘了選新馬達?</b>系統會比對數值,發現與這顆的歷史明顯不符時跳出琥珀色提醒;也可用歷史表下方的「清除導引紀錄」手動歸零(按下會先跳確認框,清除後無法復原)。</li></ol><p><b>磨合導引歷史怎麼看</b>:每列＝一場(新的在上),顯示時間、型號、輪數與該場的收斂數值。<b>由下往上看:數值一路下降＝還在進步;降到持平＝黃金點</b>——這張表就是橫幅判定的依據,讓你親眼驗證,不必憑感覺信。標 ✗ 的場(故障或手動中止)不參與判定,不用理會。</p><div class='manual-note'><b>注意</b>:導引依<b>型號</b>分開判讀,換型號自動另起爐灶;但<b>同型號的不同顆</b>,必須靠你在啟動時選「新馬達」來區分。每顆馬達的黃金點場數都不同——<b>看導引判定,不要背場數</b>。判定基於本機同條件量測;換治具、環境溫度大變後的第一場數值可能有偏移,建議多磨一場再判讀。<br><b>升級到 v3.6.0／v3.7.0 後</b>:磨合工況已更新(v3.7.0 起每輪等溫取樣),導引歷史跨版本不可比——升級後第一場啟動時請選「<b>新馬達(重新開始)</b>」重建基準。</div>"
            )},
            {"id": "standardflow", "t": "七、標準磨合流程(磨前量測 → 磨合 → 磨後量測)", "html": (
                "<p class='manual-intro'>想知道「磨合到底有沒有效、效果多大」,建議照這個三步驟標準流程。</p><p><b>① 磨前量測</b>:先跑一次「<b>AI 扭力預測</b>」(約 1~2 分鐘),記下這顆的<b>內耗％與扭力</b>——這是磨合前的基準(<b>體質％不必記</b>:磨前電刷尚未貼合、其讀值不確定度較大,前後差在 ±5 內不具判讀意義,見下表)。</p><p><b>② 磨合</b>(兩條路擇一):</p><ul><li><b>AI 智慧磨合</b>:啟動、讓它磨到自動停(「已磨合至最佳狀態」);之後看<b>磨合導引</b>橫幅決定要不要再磨一場(顯示「已到黃金點」就收工)。</li><li><b>自己的配方</b>:用「馬達磨合」(電壓／轉速模式)跑你慣用的配方——磨前磨後量測照樣對照,<b>你的配方到底磨出多少效果,數據會告訴你</b>。</li></ul><p><b>③ 磨後量測</b>:再跑一次「AI 扭力預測」,與磨前對照:</p><table class='manual-table'><thead><tr><th>指標</th><th>磨合有效的樣子</th><th>怎麼解讀</th></tr></thead><tbody><tr><td><b>磨合前／後轉速(磨合頁顯示)</b></td><td><b>上升</b></td><td><b>主判讀指標(所有馬達皆適用)</b>——同電壓下轉速提升＝接觸改善的最直接體現</td></tr><tr><td>內耗％</td><td>用過／變髒／久置:<b>明顯下降</b>;全新品:持平或微升皆正常</td><td><b>用過馬達的主判讀</b>——內部摩擦與損耗變小＝真的磨開了;下降型降到低點不再降＝已磨透(全新品判讀見下方全新品註)</td></tr><tr><td>扭力(@2.4V／3.0V)</td><td>持平或略升</td><td>扭力主要由磁鐵決定,磨合不會大改;若明顯下降屬異常,建議檢查</td></tr><tr><td>體質％</td><td>升或降都可能(±5 以內＝持平)</td><td>綜合分數;磨前量測的不確定度較大(電刷尚未貼合),<b>前後差在 ±5 以內一律視為量測散布</b>——磨合效果判讀以轉速為主</td></tr></tbody></table><div class='manual-note'><b>全新品註</b>:全新未磨的馬達,內耗％的<b>起點高低因個體與油況而異</b>——實測有磨合後<b>明顯下降</b>的,也有本來就乾淨低阻、磨後<b>持平甚至微升</b>的(電刷貼合面積變大、初生碳粉),<b>兩種都正常</b>。所以全新品<b>不要用內耗％單獨判讀磨合效果</b>,請主要看<b>磨合指標曲線下降</b>與<b>磨合前後轉速上升</b>;「內耗明顯下降＝磨開了」的判讀主要適用於<b>用過、變髒或久置的馬達</b>。</div><div class='manual-note'><b>沒有 Pro?</b>用「馬達特性量測」(全版本可用)做磨前磨後對照也行——主要看 <b>★I0(無載電流)與 T_loss 是否下降</b>;全新品判讀同上方全新品註。</div><p><b>④(建議)建立健康指紋</b>:磨合完成＝這顆的巔峰狀態——此時到「AI 智慧馬達健康管理」建立<b>健康指紋</b>,之後定期檢測就能掌握它離巔峰多遠(詳見「十二、AI 健康管理」)。</p><div class='manual-note'><b>量測溫度條件(v3.7.0 起自動處理)</b>:系統在每次量測前<b>自動暖機到同一量測基準溫度</b>,磨前／磨後量測的溫度條件已自動一致,不需手動等溫;結果頁顯示「量測溫度」可供核對。磨合剛結束若機器仍偏熱,建議稍等數分鐘再做磨後量測(溫度回到基準附近,系統會自動接手)。</div>"
            )},
            {"id": "torque", "t": "八、AI 扭力預測系統(Pro)", "html": (
                "<p class='manual-intro'>上機約 1~2 分鐘,用<b>田宮官方規格當基準</b>,推算這顆馬達的扭力與體質分數。回答「這顆到底幾分」——<b>跟官方標準比,不跟別顆比</b>。</p><p><b>操作</b>:首頁 →「AI 扭力預測系統」→ 選<b>馬達型號</b>(田宮全 15 款;型號要選對,基準才對)＋填<b>備註</b>(選填,40 字以內)→「開始預測」→ 全程自動連續量測(約 1~2 分鐘,過程中請勿觸碰馬達)→ 顯示結果頁。</p><p><b>結果頁怎麼看</b>:</p><table class='manual-table'><thead><tr><th>項目</th><th>意思</th><th>怎麼判讀</th></tr></thead><tbody><tr><td><b>預測扭力 @2.4V／@3.0V</b></td><td>這顆在兩個常用電壓下的推算扭力(mN·m)</td><td>對照下方「田宮標準」看落在目錄帶哪裡</td></tr><tr><td><b>體質 vs 田宮標準</b></td><td>扭力佔官方標準的百分比(%)</td><td><b>約 100%＝體質正常</b>;這是天生的,磨合改不了</td></tr><tr><td><b>真值電流 @2.4V／@3.0V</b></td><td>換算到官方負載條件下的等效電流(A)</td><td>與「田宮標準」電流帶對照——「照官方條件跑會吃多少電」</td></tr><tr><td><b>馬達內耗</b></td><td>空轉自耗,<b>越低越好</b></td><td>判讀見「二十、物理知識」的「馬達內耗」條(三態)</td></tr><tr><td><b>量測一致性檢核</b></td><td>本次三趟量測穩不穩</td><td>顯示「通過」才採信;「未通過」代表架設 / 感測不穩,重測</td></tr><tr><td><b>狀態判讀行(結果頁最下方)</b></td><td>機器直接給的白話結論</td><td>✅ 綠色＝內部損耗正常、可安心使用;⚠ 琥珀＝損耗偏高(建議先磨合／清潔)或讀數異常偏低(疑導電油,洗淨重測)。<b>看結論優先於看分數</b></td></tr></tbody></table><p><b>幾個一定要知道的</b>:</p><ul><li><b>這是「參考值」,精度扭力 ±3%、體質分數 ±5%</b>——判讀看落在哪個帶、看同顆前後變化,<b>別糾結單點小數</b>。</li><li><b>磨合後體質分數比全新時略低＝正常</b>(全新品磨合後電刷貼合面積變大、空轉自耗微升)——不代表磨壞;磨合效果請看「七、標準磨合流程」的判讀表。</li><li><b>以田宮官方標準為基準</b>(不是實體扭力計絕對值);官方規格是玩家共同的認知座標,這樣的分數才好互相溝通。</li><li><b>同型號不同顆分數不同＝正常</b>:反映每顆的真實個體差異,不是量錯。</li><li><b>型號務必選對</b>:選錯型號＝拿錯基準比,分數就沒意義。</li></ul><div class='manual-note'><b>注意</b>:全程自動、勿觸碰馬達;「一致性檢核未通過」時先查馬達架設與轉速感測對位再重測;高溫會自動中止。</div>"
            )},
            {"id": "test", "t": "九、馬達測試", "html": (
                "<p class='manual-intro'>單階段即時觀察,<b>不寫入紀錄</b>。</p><p>首頁 →「馬達測試程式」→ 設 <b>電壓(0.6~4.0V,預設 1V)/ 運轉時間(10~600 秒,預設 60)/ 方向 / 穩定電流容差(2~50mA)/ 判停標準</b>(純時間限制 或 智慧穩定＝電流提前穩定即提早結束,預設後者;此設定與磨合頁<b>各自獨立</b>)→「啟動」→ 看即時數據與圖表(含 <b>KV</b> 即時值),到時或穩定即自動停。</p><div class='manual-note'><b>注意</b>:電壓上限 4.0V 保護馬達,勿繞過;反轉接正轉前先按一次「停止」。機器未達量測基準溫度時,系統自動先暖機再開始(計時不含暖機);環境溫度過低／過高會提示「數據僅供參考」。</div>"
            )},
            ...
            {"id": "health", "t": "十二、AI 智慧馬達健康管理(Pro)", "html": (
                "<p class='manual-intro'>為每顆馬達建立<b>健康指紋</b>——磨合完成時的巔峰狀態快照——之後定期重測比對,機器直接告訴你這顆馬達「維持得如何、哪裡開始退化」。<b>跟自己比,不跟別顆比</b>。</p><p><b>正確使用順序(重要)</b>:先把馬達<b>磨合完成</b>(建議用 AI 智慧磨合磨到自動停),再建立指紋。指紋代表這顆馬達的最佳狀態,之後每次檢測都是在問「離巔峰多遠了」。</p><ol><li>首頁 →「AI 智慧馬達健康管理」→「+ 新增馬達」。</li><li>填 <b>馬達型號</b>(下拉,必選——診斷需要對照該型號的標準)+ <b>備註</b>(選填)。</li><li>確認 → 自動跑約 <b>4.5 分鐘</b> 建立指紋(特性量測 3 趟 → 電刷接觸 → 換向紋波紀錄)。若機器提示「<b>可能尚未磨透</b>」:代表這顆的內部損耗還偏高,建議先跑 AI 智慧磨合,完成後重建指紋。</li></ol><p>之後每張卡片可做:</p><ul><li><b>完整檢測</b>(約 4.5 分):<b>磁力／內部損耗／電刷接觸</b>三面向對指紋比對,最完整。</li><li><b>快速檢測</b>(約 1.5 分):只比對內部損耗,適合日常快篩。</li></ul><p>結果頁顯示分數(0~100,只有「往壞的方向」才扣分)、等級(Optimal / Acceptable / Warning / Critical)、趨勢圖與白話建議。溫度差異會自動補償——冬天建的指紋夏天檢測不會誤判。</p><p><b>什麼時候要重建指紋</b>:洗馬達／深度保養之後、或檢測顯示「比指紋更順了」時——把新狀態設為新基準。舊指紋會保留在歷史裡,成為這顆馬達的生命履歷。</p><div class='manual-note'><b>注意</b>:每顆最多 50 次歷史,整機最多 20 顆;檢測中勿斷電 / 勿按其他鈕;高溫會自動中止;舊版建立的指紋需重建一次才能使用新診斷;M1 版按鈕灰色(點下提示升級)。</div>"
            )},
            {"id": "bearing", "t": "十三、軸承阻力測試", "html": (
                "<p class='manual-intro'>量軸承順暢度,任何版本可用。<b>完全靜止時間越久 → 軸承越順</b>。</p><p>首頁 →「軸承阻力測試」→ 選測試電壓(<b>2.4V 或 3V</b>)→「開始測試」→ 機器加速到該電壓 → 穩定轉速 5 秒 → <b>直接斷電(不剎車)</b> → 計時到完全靜止 → 顯示 <b>完全靜止時間</b>。</p><div class='manual-note'><b>注意</b>:馬達空載慣性小,本測試<b>只顯示時間、不做好壞評定</b>,請拿同一顆馬達不同時期、或不同馬達的時間互比;需轉速感應器正常;建議測 2~3 次看重複性。溫度低時油較稠、時間會偏短——系統每次<b>自動先暖機到量測基準溫度再測</b>,確保不同時期、不同馬達的結果可比。</div>"
            )},
        ...
        "en": [
            ...
            {"id": "smartbreakin", "t": "6. AI Smart Break-in (Pro)", "html": (
                "<p class='manual-intro'>One-tap fully-automatic break-in: the machine runs, measures and stops itself when done; a built-in <b>Break-in Guide</b> reads across sessions to answer break-in's hardest question — &quot;<b>how many sessions does this motor actually need?</b>&quot;</p><p><b>How to run</b>: Home → &quot;AI Smart Break-in&quot; → pick <b>motor model</b> (all 15 Tamiya) + <b>note</b> (optional, up to 40 chars) + <b>time cap</b> (until done / 2h / 3h / 4h) → &quot;Start,&quot; the rest is automatic — the system runs at progressive speeds while measuring, and <b>stops automatically once it detects steady state (no more improvement this session)</b> with a beep.</p><ul><li>The live data and charts at the top are <b>identical to the break-in page</b>; below it shows: current round / current action / elapsed time / convergence progress, plus a <b>break-in metric curve</b> (per-round points + moving average; a flattening curve = this session near done).</li><li>The motor <b>alternates forward/reverse</b> (bedding both brush faces symmetrically) and measures periodically; rhythmic changes in sound and speed are normal.</li><li><b>Isothermal sampling</b>: the first round is a <b>warm-up baseline round</b> (break-in runs normally and sets this session's temperature baseline, not counted in the data or curve); from the second round on, before each round's measurement the system waits for the machine to return to the temperature baseline (up to a few minutes if needed; <b>the motor pausing during this is normal</b>) — so in cold or hot environments alike, every round's judgment sample is taken at the same temperature, and the verdict is unaffected by ambient temperature.</li><li>Pressing &quot;Stop&quot; <b>stops immediately</b>; data from completed rounds is kept as usual; a manually-stopped session is logged in the guide history but <b>does not count toward the verdict</b> (marked ✗).</li></ul><h3>Break-in Guide — answering &quot;how many sessions&quot;</h3><p><b>One session done = the motor is broken in? Not necessarily.</b> Each auto session stops at its own steady state, but a new motor usually needs <b>several consecutive sessions</b> to truly arrive; after that, more grinding brings no gain and starts consuming brush and magnet life (see &quot;20. Physics&quot;: &quot;demagnetization&quot; and &quot;break-in sweet spot&quot;). The Guide's job is to catch <b>the session to stop at</b>.</p><p><b>How it works</b>: after each session the system records its convergence result; the next session is auto-compared with all prior ones — still clearly dropping = still improving; flat for several in a row = improvement exhausted = <b>break-in sweet spot</b>. The verdict shows directly on the panel banner:</p><table class='manual-table'><thead><tr><th>Banner</th><th>Meaning</th><th>What to do</th></tr></thead><tbody><tr><td>Guide observing — one more session to start judging</td><td>Not enough sessions yet to read</td><td>Keep breaking in normally</td></tr><tr><td>Still improving, worth continuing (already saved x mA vs start)</td><td>This one still has headroom</td><td><b>One more session is worth it</b></td></tr><tr><td>Sweet spot reached, stop recommended — little gain from more</td><td>Improvement exhausted, this is it</td><td><b>Stop; this one is done</b></td></tr><tr><td>Overshot — you're now grinding away life, stop immediately</td><td>The sweet spot passed a few sessions ago</td><td>Stop now; don't grind this one again</td></tr></tbody></table><p><b>When to look / press</b>:</p><ol><li><b>First session of a new motor</b>: you'll only see &quot;observing&quot; — the first session builds this motor's baseline, which is normal.</li><li><b>Glance at the banner after each session</b>: &quot;still improving&quot; → start another; &quot;<b>sweet spot reached</b>&quot; → stop — this motor's break-in is done, put it in the car.</li><li><b>The &quot;guide record&quot; prompt at start</b> (only when records exist): continuing the <b>same motor</b> → &quot;continue same motor&quot; (default); a <b>different</b> motor is now fitted → &quot;<b>new motor (restart)</b>,&quot; which resets the guide record — not resetting means comparing against another motor's record and misjudging.</li><li><b>Forgot to pick new motor?</b> The system compares values and pops an amber reminder when they clearly mismatch this one's history; you can also reset manually via &quot;clear guide record&quot; below the history table (a confirm box appears; clearing is irreversible).</li></ol><p><b>Reading the guide history</b>: each row = one session (newest on top), showing time, model, rounds and that session's convergence value. <b>Read bottom-up: values dropping all the way = still improving; leveling off = sweet spot</b> — this table is the basis for the banner verdict, so you can verify it with your own eyes. Rows marked ✗ (fault or manual stop) don't count toward the verdict; ignore them.</p><div class='manual-note'><b>Note</b>: the guide judges per <b>model</b> and starts fresh on a model change; but <b>different units of the same model</b> must be distinguished by picking &quot;new motor&quot; at start. Every motor's sweet-spot session count differs — <b>read the guide verdict, don't memorize a session count</b>. Verdicts are based on same-condition measurement on this machine; the first session's values may shift after a jig change or big temperature change, so break in one more before reading.<br><b>After upgrading to v3.6.0 / v3.7.0</b>: the break-in profile changed (v3.7.0 adds isothermal sampling each round), so guide history is not comparable across versions — on the first session after upgrading, pick &quot;<b>new motor (restart)</b>&quot; to rebuild the baseline.</div>"
            )},
            {"id": "standardflow", "t": "7. Standard Break-in Flow (Before → Break-in → After)", "html": (
                "<p class='manual-intro'>To know &quot;whether break-in actually worked, and by how much,&quot; follow this three-step standard flow.</p><p><b>① Measure before</b>: first run <b>AI Torque Prediction</b> (about 1–2 min) and note this motor's <b>internal loss % and torque</b> — the pre-break-in baseline (<b>no need to note constitution %</b>: before break-in the brushes aren't bedded yet, so its reading has high uncertainty and a before/after gap within ±5 is not meaningful — see the table).</p><p><b>② Break in</b> (pick one path):</p><ul><li><b>AI Smart Break-in</b>: start it and let it stop itself (&quot;broken in to best state&quot;); then read the <b>Break-in Guide</b> banner to decide on another session (&quot;sweet spot reached&quot; = done).</li><li><b>Your own recipe</b>: use &quot;Break-in&quot; (voltage / speed mode) with your usual recipe — before/after measurements still compare, so <b>the data tells you exactly how much your recipe achieved</b>.</li></ul><p><b>③ Measure after</b>: run AI Torque Prediction again and compare with before:</p><table class='manual-table'><thead><tr><th>Metric</th><th>Effective break-in looks like</th><th>How to read</th></tr></thead><tbody><tr><td><b>Pre/post-break-in RPM (break-in page)</b></td><td><b>Up</b></td><td><b>The primary metric (all motors)</b> — higher RPM at the same voltage = the most direct sign of improved contact</td></tr><tr><td>Internal loss %</td><td>Used / dirty / long-stored: <b>clear drop</b>; brand-new: flat or a slight rise is normal</td><td><b>The primary metric for used motors</b> — less internal friction/loss = genuinely opened up; a dropping one that bottoms out = fully broken in (for brand-new see the note below)</td></tr><tr><td>Torque (@2.4V / 3.0V)</td><td>Flat or slightly up</td><td>Torque is set mainly by the magnet; break-in won't change it much; a clear drop is abnormal — inspect</td></tr><tr><td>Constitution %</td><td>May rise or fall (within ±5 = flat)</td><td>A composite score; the pre-break-in reading has high uncertainty (brushes not bedded), so <b>a before/after gap within ±5 is measurement spread</b> — judge the break-in effect mainly by RPM</td></tr></tbody></table><div class='manual-note'><b>Brand-new note</b>: for a brand-new un-run motor, the <b>starting internal-loss % varies by unit and lubrication</b> — some drop clearly after break-in, others are already clean and low-resistance and stay <b>flat or even rise slightly</b> (larger brush contact area, fresh carbon); <b>both are normal</b>. So for brand-new motors <b>don't judge the break-in effect by internal loss % alone</b> — mainly watch the <b>break-in metric curve dropping</b> and <b>pre/post RPM rising</b>. The &quot;clear internal-loss drop = opened up&quot; reading applies mainly to <b>used, dirtied or long-stored motors</b>.</div><div class='manual-note'><b>No Pro?</b> You can also compare before/after with <b>Motor Characterization</b> (all versions) — mainly watch whether <b>★I0 (no-load current) and T_loss drop</b>; for brand-new motors, same as the note above.</div><p><b>④ (Recommended) Build a health fingerprint</b>: break-in complete = this motor's peak state — now open &quot;AI Motor Health Manager&quot; and build a <b>health fingerprint</b>, so periodic checks later show how far it has drifted from peak (see &quot;12. AI Health Manager&quot;).</p><div class='manual-note'><b>Measurement temperature (auto-handled since v3.7.0)</b>: before every measurement the system <b>auto-warms up to the same measurement baseline temperature</b>, so the before and after measurements are already at matched temperatures — no manual waiting; the result page shows &quot;measurement temperature&quot; for cross-checking. If the machine is still hot right after break-in, wait a few minutes before the after measurement (once temperature is near baseline the system takes over automatically).</div>"
            )},
            {"id": "torque", "t": "8. AI Torque Prediction (Pro)", "html": (
                "<p class='manual-intro'>About 1–2 minutes on the machine; using <b>Tamiya's official specs as the baseline</b>, it estimates this motor's torque and constitution score. It answers &quot;what's this one's score&quot; — <b>compared to the official standard, not to other units</b>.</p><p><b>How to run</b>: Home → &quot;AI Torque Prediction&quot; → pick <b>motor model</b> (all 15 Tamiya; pick correctly, or the baseline is wrong) + <b>note</b> (optional, up to 40 chars) → &quot;Start prediction&quot; → fully automatic continuous measurement (about 1–2 min; do not touch the motor) → shows the result page.</p><p><b>Reading the result page</b>:</p><table class='manual-table'><thead><tr><th>Item</th><th>Meaning</th><th>How to read</th></tr></thead><tbody><tr><td><b>Predicted torque @2.4V / @3.0V</b></td><td>Estimated torque at the two common voltages (mN·m)</td><td>Compare with the &quot;Tamiya standard&quot; below to see which band it lands in</td></tr><tr><td><b>Constitution vs Tamiya standard</b></td><td>Torque as a percent of the official standard (%)</td><td><b>~100% = normal constitution</b>; this is innate, break-in can't change it</td></tr><tr><td><b>True current @2.4V / @3.0V</b></td><td>Equivalent current converted to the official load condition (A)</td><td>Compare with the &quot;Tamiya standard&quot; current band — &quot;how much it draws under official conditions&quot;</td></tr><tr><td><b>Motor internal loss</b></td><td>Self-consumption while spinning; <b>lower is better</b></td><td>See &quot;20. Physics&quot;: &quot;motor internal loss&quot; (three states)</td></tr><tr><td><b>Measurement consistency check</b></td><td>Whether this run's three passes are stable</td><td>Trust it only when it shows &quot;pass&quot;; &quot;fail&quot; means unstable mounting / sensing — re-measure</td></tr><tr><td><b>Status verdict line (bottom of result page)</b></td><td>The machine's plain-language conclusion</td><td>✅ green = internal loss normal, safe to use; ⚠ amber = loss high (break in / clean first) or abnormally low reading (suspect conductive oil — wash and re-measure). <b>The verdict outranks the score</b></td></tr></tbody></table><p><b>A few things you must know</b>:</p><ul><li><b>These are &quot;reference values,&quot; torque ±3%, constitution score ±5%</b> — read which band it's in and same-unit before/after changes, <b>don't fixate on single-point decimals</b>.</li><li><b>A slightly lower constitution score after break-in than when brand new = normal</b> (a broken-in brand-new motor has a larger brush contact area and slightly higher idle draw) — not damage; for the break-in effect see the table in &quot;7. Standard Break-in Flow.&quot;</li><li><b>Based on Tamiya's official standard</b> (not an absolute torque-meter value); the official spec is players' shared coordinate, which makes scores comparable in conversation.</li><li><b>Different units of the same model scoring differently = normal</b>: it reflects real unit-to-unit variation, not measurement error.</li><li><b>Pick the right model</b>: a wrong model = wrong baseline = meaningless score.</li></ul><div class='manual-note'><b>Note</b>: fully automatic, don't touch the motor; on &quot;consistency check failed,&quot; check motor mounting and speed-sensor alignment before re-measuring; overheating auto-aborts.</div>"
            )},
            {"id": "test", "t": "9. Motor Test", "html": (
                "<p class='manual-intro'>Single-stage live observation, <b>not written to history</b>.</p><p>Home → &quot;Motor Test&quot; → set <b>voltage (0.6–4.0V, default 1V) / run time (10–600 s, default 60) / direction / stable-current tolerance (2–50mA) / stop criterion</b> (Time-only, or Smart stable = ends early once current settles, default; this setting is <b>independent</b> of the break-in page) → &quot;Start&quot; → watch live data and charts (incl. live <b>KV</b>); it auto-stops at time or on stability.</p><div class='manual-note'><b>Note</b>: the 4.0V ceiling protects the motor, don't bypass it; press &quot;Stop&quot; once before switching reverse to forward. If the machine is below the measurement baseline temperature it auto-warms up first before starting (timing excludes warm-up); too-low / too-high ambient temperature shows &quot;data for reference only.&quot;</div>"
            )},
            ...
            {"id": "health", "t": "12. AI Motor Health Manager (Pro)", "html": (
                "<p class='manual-intro'>Build a <b>health fingerprint</b> for each motor — a snapshot of its peak state at break-in completion — then re-measure periodically so the machine tells you directly &quot;how well it's holding up, and where it's starting to degrade.&quot; <b>Compare against itself, not other units.</b></p><p><b>Correct order (important)</b>: first <b>finish breaking in</b> the motor (ideally with AI Smart Break-in to auto-stop), then build the fingerprint. The fingerprint is this motor's best state, and every later check asks &quot;how far from peak now.&quot;</p><ol><li>Home → &quot;AI Motor Health Manager&quot; → &quot;+ Add motor.&quot;</li><li>Fill <b>motor model</b> (dropdown, required — diagnosis needs that model's standard) + <b>note</b> (optional).</li><li>Confirm → runs about <b>4.5 minutes</b> to build the fingerprint (3-pass characterization → brush contact → commutation-ripple record). If it warns &quot;<b>may not be fully broken in</b>&quot;: this one's internal loss is still high — run AI Smart Break-in first, then rebuild the fingerprint.</li></ol><p>Then each card can run:</p><ul><li><b>Full check</b> (about 4.5 min): compares <b>magnetism / internal loss / brush contact</b> against the fingerprint — the most complete.</li><li><b>Quick check</b> (about 1.5 min): compares internal loss only, for daily screening.</li></ul><p>The result page shows a score (0–100, points deducted only for changes &quot;in the bad direction&quot;), a grade (Optimal / Acceptable / Warning / Critical), a trend chart and plain advice. Temperature differences are auto-compensated — a fingerprint built in winter won't misjudge in summer.</p><p><b>When to rebuild the fingerprint</b>: after washing / deep maintenance, or when a check shows &quot;smoother than the fingerprint&quot; — set the new state as the new baseline. The old fingerprint stays in history as this motor's life record.</p><div class='manual-note'><b>Note</b>: up to 50 records per motor, 20 motors per machine; during a check don't cut power / press other buttons; overheating auto-aborts; fingerprints built on an old version need one rebuild to use the new diagnosis; on M1 the button is greyed out (tapping prompts to upgrade).</div>"
            )},
            {"id": "bearing", "t": "13. Bearing Resistance Test", "html": (
                "<p class='manual-intro'>Measures bearing smoothness; works on any version. <b>Longer time to full stop → smoother bearing.</b></p><p>Home → &quot;Bearing Resistance Test&quot; → pick test voltage (<b>2.4V or 3V</b>) → &quot;Start&quot; → the machine accelerates to that voltage → holds a steady speed 5 s → <b>cuts power directly (no braking)</b> → times to full stop → shows the <b>full-stop time</b>.</p><div class='manual-note'><b>Note</b>: the motor's no-load inertia is small, so this test <b>only shows time, no good/bad rating</b>; compare the same motor over time, or different motors, by their times; needs a working speed sensor; test 2–3 times for repeatability. When cold the oil is thicker and the time runs short — the system <b>auto-warms up to the measurement baseline temperature before each test</b>, so results are comparable across times and motors.</div>"
            )},
            {"id": "brush", "t": "14. Brush Contact Stability Test", "html": (
                "<p class='manual-intro'>Measures whether brush contact is even; about 35 s, works on any version. <b>Uneven contact → current jitter → CV rises.</b></p><p>Home → &quot;Brush Contact Stability Test&quot; → &quot;Start&quot; (<b>no need to turn the shaft by hand</b>). The flow is fixed at <b>1V</b>: voltage ramps 0→1V over 10 s → holds 1V for 5 s → samples current at 1V for 20 s → shows the <b>current coefficient of variation CV%</b>.</p><p><b>Reading CV%</b>: CV is the current jitter at fixed voltage — <b>a relative comparison tool, no absolute good/bad line</b> (models differ innately). Suggested uses:</p><ul><li>Same motor <b>before vs after break-in</b>: CV drops = brushes bedded, contact improved.</li><li>Same motor <b>tracked over time</b>: CV rising = brush wear / dirty commutator.</li><li><b>Motor A vs B</b> relative comparison.</li></ul><div class='manual-note'><b>Note</b>: fixed 1V, fixed flow, for fair cross-run / cross-motor comparison; test 2–3 times for repeatability.</div>"
            )},
        ...
        "ja": [
            ...
            {"id": "charexp", "t": "四、モーター特性測定", "html": (
                "<p class='manual-intro'>モーターの特性パラメータと損失を全自動で測定し、<b>参考データを提示・評点や順位付けはしません</b>——同型モーター同士を比較して自分で判断。システムが<b>連続 3 回測定して平均</b>、約 5 分、穏やかな処理で新品未慣らしモーターにも適します。</p><p><b>操作</b>:ホーム →「モーター特性測定」→ 方向選択(既定は正転)+ <b>モーター型番</b>選択(16 種プルダウン)+ <b>備考</b>入力(任意、40 字以内)→「測定開始」→ 全自動(回数と進捗を表示)、途中でモーターに触れない → 完了でデータ表を表示し<b>測定履歴に自動保存</b>。</p><p><b>データの見方(表内に各項の方向を表示)</b>:</p><ul><li><b>★ I0(無負荷電流)</b>:同回転数で<b>低いほど良い</b>——再現性が最も高く、選別の第一指標。</li><li><b>★ Km(品質係数)/ ★ T_loss(損失トルク)</b>:補助指標(Km は高いほど・T_loss は低いほど良い)。</li><li><b>Ke(磁気回路の強さ)</b>:絶対的な良し悪しなし——数値の近いモーター同士の比較で正確(高い=トルク寄り、低い=回転寄り)。</li><li><b>KV(回転数定数、rpm/V)</b>:Ke から換算——KV × 電圧 ≈ 無負荷回転数(例 KV 14000 × 3V ≈ 42000 RPM)、市販表示との対照に便利。</li><li><b>R(内部抵抗)</b>:低いほど良い。</li><li><b>始動電圧</b>:参考のみ——単発ではばらつきが大きく、同一モーターの長期健康対照向き(後日明らかに上昇=ベアリング／ブラシ劣化の警告)。</li><li><b>損失フィット R² / Ke ドリフト</b>:データ品質指標(1 に近く、ドリフトが低いほど信頼できる)。</li><li><b>3 回の一致性</b>:各回 I0 の最大差;⚠ 表示はモーター不安定または取付緩みを示し、そのデータは参考程度とし、点検して再測定を推奨。</li></ul><p><b>測定履歴</b>:測定ページ →「測定履歴」。成功した測定は自動保存(全指標と各回転点データ含む)、最大 <b>50 件</b>、満杯で最古を自動削除;詳細閲覧・削除可。</p><p><b>使い方の目安</b>:</p><ul><li>同ロットの新品を 1 個ずつ測定 → ★I0 で素性の良い個体を選んで慣らしに投入(慣らし工数の節約)。</li><li>慣らし後に<b>同じ個体を再測定</b>して対照:損失系は下がるはず(慣らし有効);Ke / R はほぼ不変。</li><li>異なるモーターの比較は<b>近い温度</b>で;取付直後や冷間再起動後の初回データは高めに出ることがあり、2 回目以降を基準に。</li></ul><div class='manual-note'><b>測定基準温度の統一(v3.7.0 から全自動)</b>:温度は測定に影響します——システムは各測定前に機器が<b>測定基準温度</b>に達したか確認し、未達なら先に自動ウォームアップ(最大約 3 分、「ウォームアップ中」表示)してから開始し、毎回同じ温度で測定します(AI トルク予測・健康管理も同様に保護);環境が低すぎ／高すぎる場合は「データは参考のみ」と表示し、テストは続行。さらに<b>磁力系の数値(Ke／Km／KV)は 25°C 基準値へ自動換算</b>され、異なる日・季節の結果を直接比較できます。</div><div class='manual-note'><b>安全保護(全自動)</b>:モーターを駆動するあらゆる処理(慣らし／テスト／特性測定／ベアリング／ブラシ／健康検査)中、全自動で監視し異常時は直ちに停止して説明を表示します——モーター未検出(< 3 秒)、ロック(< 5 秒)、回転信号なし(< 4 秒)、出力短絡(緊急停止 < 0.15 秒)、高温(即停止 + ロック)。作動時はブザーが 4 長音。<b>供給規格 DC 5V／6A を下回らないこと</b>;供給不足時は上部に琥珀色の警告「設定電圧に達しない可能性」を表示。</div><div class='manual-note'><b>注意</b>:測定失敗(始動せず / ロックタイムアウト / 過電流)は原因を表示、再試行で可。繰り返し失敗、または待機時に回転数が 0 でない場合は、回転センサーと反射板の位置合わせを確認してください。</div>"
            )},
            {"id": "breakin", "t": "五、モーター慣らし(2 モード)", "html": (
                "<p class='manual-intro'>低速で長時間運転し、ブラシと整流子を最適な接触まで馴染ませます。慣らしページ上部に<b>2 つのモードボタン</b>(運転中は切替不可)。</p><table class='manual-table'><thead><tr><th>モード</th><th>向いている人</th><th>ひとこと</th></tr></thead><tbody><tr><td><b>電圧モード</b></td><td>自分の配合を持つ人</td><td>各段階を固定電圧で運転(従来方式)</td></tr><tr><td><b>回転数モード (Beta)</b></td><td>回転数を基準にしたい人</td><td>各段階を設定回転数に自動制御、電圧安定で自動的に次段階へ</td></tr></tbody></table><div class='manual-note'>「ワンタップ全自動・完了で自動停止」を求める人は、ホームの独立機能「<b>AI スマート慣らし</b>」(Pro、「六、AI スマート慣らし」参照)をご利用ください。</div><h3>電圧モード</h3><p>ホーム →「モーター慣らしプログラム」→ <b>型番</b>選択(16 種)+ <b>備考</b>(40 字以内)→ <b>判定基準</b>選択(純時間制限=各段階を設定時間まで運転;スマート安定=電流が早めに安定したら次段階へ、時間は上限)→ <b>慣らし回数</b>選択(1~6、既定 1)→ 各回 10 段階のパラメータを確認(複数回はタブで切替;数値欄を直接タップして編集)→「開始」。</p><ul><li>各段階:電圧(0.6~4.0V、<b>0=スキップ</b>)/ 方向 / 慣らし秒(10~600、既定 60)/ 冷却秒(10~600、既定 60)/ 安定電流公差(2~50mA)。</li><li>複数回は「<b>この回を以降すべてにコピー</b>」で素早く入力;回数横に<b>推定総時間</b>を即時表示。</li><li>範囲外の入力は自動的に上下限へ補正し一度点滅して知らせます。</li></ul><div class='manual-note'><b>ポイント</b>:新品モーターの慣らしではブラシと整流子がまだ整形されていないため、公差の設定を回数ごとに大から小へ徐々に絞ることができます。</div><h3>回転数モード (Beta)</h3><p>電圧モードと同じ段階表ですが、第 1 列が<b>目標回転数</b>(6000~30000、1000 刻み、<b>0=スキップ</b>)に変わります——機器が電圧を自動調整してモーターをその回転数に固定します。</p><ul><li><b>判定基準</b>(厳格 / 標準 / 緩め):その回転数で駆動電圧が安定すれば当段階を馴染んだと見なし早めに次段階へ;慣らし秒は上限保護。厳格=より安定を要求(深く・長め);緩め=早めに次段階(新品が段階を通過できない場合は緩めに)。</li><li>段階結果欄に<b>安定電圧</b>を表示;目標回転数に届かない場合は安全上限電圧で時間いっぱい運転し「<b>未達</b>」と表示(通常は目標が高すぎるか、モーター／治具の異常)。</li><li>方向・時間・回数・コピー等の操作は電圧モードと同じ。</li></ul><h3>共通の動作</h3><p>各段階の流れ:ソフトスタート → 運転中 → ソフトストップ → 冷却中 → 次段階;1 回終わると自動で次の回へ(「x/y 回目 · 段階 c」を表示)。全完了 → 3 ビープ → 履歴に自動保存(電圧／回転数モード、各回各段階の結果を含む)。</p><p><b>運転中に押せるボタン</b>:</p><table class='manual-table'><thead><tr><th>ボタン</th><th>動作</th></tr></thead><tbody><tr><td>一時停止</td><td>ソフトストップ後に<b>一時停止</b>、ボタンが「<b>再開</b>」に変わる;再度押すと<b>中断点から続行</b>——段階で走った秒数は保持(再計算なし)、冷却カウントも継続。一時停止に時間制限なし;<b>一時停止中は他のボタンは全ロック、「再開」のみ有効</b></td></tr><tr><td>停止</td><td>ソフトストップ後に終了、記録に「ユーザー中止」</td></tr><tr><td>ホーム</td><td>ホームへ戻る、慣らしは<b>バックグラウンドで継続</b></td></tr></tbody></table><div class='manual-note'><b>一時停止について</b>:一時停止中はモーターが停止して冷え、再開はその段階に追加の冷却を 1 回挟むのと同じで慣らしに無害;再開後は段階の「安定判定」を採り直します(冷熱状態の混同を防ぐ)。<b>電源を切ると一時停止の進捗は保持されません</b>——途中の断電は中止扱い。(「ゼロ」は待機時のみ押せ、前回運転の表示記録をクリア。)</div><div class='manual-note'><b>注意</b>:運転中は全設定がロック(一時停止中も同様);途中で電源を切らない(データ消失);高温時は自動停止して保存;素早い動作確認は各段階の時間を短く。リアルタイムデータに <b>KV(rpm/V)</b>＝現在回転数÷モーター電圧 の 1 枠が加わり、モーター状態の素早い比較に使えます;さらに「<b>本段階運転 x / y 秒</b>」が本段階の慣らし進捗を直接表示——<b>全速運転の秒数のみ累計</b>(ソフトスタート／ソフトストップ／一時停止は不算入)、y 秒で次段階へ。</div><div class='manual-note'><b>重要:測定／慣らし時に「導電潤滑オイル」を使用しないでください。</b>「導電」を謳う慣らしオイル(導電カーボン粉入り)は、注入直後に回転数が急上昇・電圧が急降下します。<b>これは本当に慣らせたのではなく、カーボン粉が一時的に導通している見かけです</b>。本機のすべての判定(安定電流 / 回転数 / 内部抵抗)はブラシと整流子の<b>本当の接触状態</b>を測ります。導電オイルは読み値を楽観側に歪めます(この種のオイルの説明書自身も「洗浄後に安定し、より良く回る」と注記)。オイルを洗い落とすと元に戻ります。正しい方法:一切使わない;既に使った場合は徹底的に洗浄してから測定してください。通常のベアリング微量潤滑は影響を受けず、この警告は「導電性慣らしオイル」のみが対象です。</div>"
            )},
            {"id": "smartbreakin", "t": "六、AI スマート慣らし(Pro)", "html": (
                "<p class='manual-intro'>ワンタップ全自動慣らし:機器が自分で慣らし・測定・完了で自動停止;内蔵の<b>慣らしガイド</b>が複数回にわたって判読し、慣らしで最も難しい問い——「<b>このモーターは結局何回慣らせば十分か?</b>」に答えます。</p><p><b>操作</b>:ホーム →「AI スマート慣らし」→ <b>型番</b>選択(タミヤ全 15 種)+ <b>備考</b>(任意、40 字以内)+ <b>時間上限</b>選択(完了まで／2h／3h／4h)→「開始」、あとは全自動——漸進的な回転数で慣らしつつ継続測定し、<b>定常状態(この回はこれ以上進歩しない)を検出すると自動停止</b>してビープで知らせます。</p><ul><li>上部のリアルタイムデータと図表は<b>慣らしページと完全に同じ</b>;下部に:現在のラウンド／現在の動作／経過時間／収束進捗、および<b>慣らし指標曲線</b>(各ラウンドの点＋移動平均、曲線が平ら=この回は完了間近)。</li><li>途中でモーターは<b>正逆交互</b>に慣らされ(ブラシ両面を対称に馴染ませる)、周期的に自動測定、音と回転数の規則的変化は正常です。</li><li><b>等温サンプリング</b>:第 1 ラウンドは<b>ウォームアップ基準ラウンド</b>(慣らしは通常どおり進み、この回の温度基準を確立、データと曲線には含めない);第 2 ラウンド以降は各ラウンドの測定前に、システムが機器の温度基準への復帰を自動で待ってから測定(必要なら最大数分;<b>その間モーターが停止するのは正常</b>)——寒冷でも高温環境でも、各ラウンドの判定サンプルは同一温度で取得され、判定は環境気温の影響を受けません。</li><li>「停止」を押すと<b>即停止</b>、完了済みラウンドのデータは通常どおり保持;手動停止した回はガイド履歴に載りますが<b>判定には参加しません</b>(✗ 表示)。</li></ul><h3>慣らしガイド——「何回か」に答える</h3><p><b>1 回終わった=慣らせた?とは限りません。</b>自動慣らしは各「回」で定常に達すると止まりますが、新品モーターは通常<b>連続して数回</b>で本当に仕上がります;仕上がった後さらに回すと進歩は無く、ブラシと磁石の寿命を削り始めます(「二十、物理知識」の「減磁」と「慣らしのゴールデンポイント」参照)。ガイドの役割は「<b>止めるべきその回</b>」を捉えることです。</p><p><b>仕組み</b>:1 回終わるごとに収束結果を記録;次の回が終わると過去各回と自動比較——まだ明確に下降=進歩中;数回連続で横ばい=進歩が尽きた=<b>慣らしのゴールデンポイント</b>。判定はパネルのバナーに直接表示:</p><table class='manual-table'><thead><tr><th>バナー表示</th><th>意味</th><th>すべきこと</th></tr></thead><tbody><tr><td>ガイド観察中——もう 1 回で判定開始</td><td>回数がまだ足りず判読不可</td><td>通常どおり続行</td></tr><tr><td>まだ進歩中、続けてお得(初期比 x mA 節約)</td><td>この個体はまだ伸びる</td><td><b>もう 1 回がお得</b></td></tr><tr><td>ゴールデンポイント到達、停止推奨——追加の伸びは僅か</td><td>進歩は尽きた、今がその時</td><td><b>停止、この個体は完了</b></td></tr><tr><td>行き過ぎ——今は寿命を削っています、直ちに停止</td><td>ゴールデンポイントは数回前に出現済み</td><td>直ちに停止、この個体はもう回さない</td></tr></tbody></table><p><b>見るタイミング・押すタイミング</b>:</p><ol><li><b>新品の 1 回目</b>:「観察中」しか出ません——1 回目はこの個体の基準作りで正常です。</li><li><b>以降は各回終了ごとにバナーを一目</b>:「まだ進歩中」なら次を開始;「<b>ゴールデンポイント到達</b>」なら停止——この個体の慣らしは完了、そのまま車へ。</li><li><b>開始時の「ガイド記録」質問</b>(記録がある時のみ表示):<b>同一個体</b>を続けて慣らす→「同一個体で続行」(既定);<b>別の</b>モーターに交換した→「<b>新しいモーター(最初から)</b>」を選択、ガイド記録がゼロから再累積——ゼロにしないと別個体の記録と混ざり判定が狂います。</li><li><b>新しいモーターを選び忘れた?</b>数値がこの個体の履歴と明らかに合わないと琥珀色の注意が出ます;履歴表下の「ガイド記録をクリア」で手動リセットも可(確認ダイアログが出て、クリア後は復元不可)。</li></ol><p><b>慣らしガイド履歴の見方</b>:各行=1 回(新しいものが上)、時刻・型番・ラウンド数・その回の収束数値を表示。<b>下から上へ:数値が下がり続け=まだ進歩中;横ばいに落ちる=ゴールデンポイント</b>——この表がバナー判定の根拠で、自分の目で確認できます。✗ 表示の回(故障・手動中止)は判定に参加せず、無視して構いません。</p><div class='manual-note'><b>注意</b>:ガイドは<b>型番</b>ごとに判読し、型番変更で自動的に別枠;ただし<b>同型番の別個体</b>は、開始時に「新しいモーター」を選んで区別する必要があります。各モーターのゴールデンポイント回数は異なる——<b>ガイドの判定を見て、回数を暗記しない</b>。判定は本機の同条件測定に基づき、治具変更や環境温度の大きな変化後の 1 回目は数値がずれることがあるため、もう 1 回慣らしてから判読を。<br><b>v3.6.0／v3.7.0 にアップグレード後</b>:慣らし工況が更新され(v3.7.0 から毎ラウンド等温サンプリング)、ガイド履歴はバージョンをまたぐと比較不可——アップグレード後の 1 回目開始時は「<b>新しいモーター(最初から)</b>」を選んで基準を再構築してください。</div>"
            )},
            {"id": "standardflow", "t": "七、標準慣らしフロー(慣らし前測定 → 慣らし → 慣らし後測定)", "html": (
                "<p class='manual-intro'>「慣らしが本当に効いたか、どれだけ効いたか」を知りたいなら、この 3 ステップの標準フローを推奨します。</p><p><b>① 慣らし前測定</b>:まず「<b>AI トルク予測</b>」を 1 回(約 1~2 分)実行し、この個体の<b>内耗％とトルク</b>を記録——これが慣らし前の基準(<b>素性％は記録不要</b>:慣らし前はブラシが未貼合で読み値の不確かさが大きく、前後差 ±5 以内は判読の意味がない、下表参照)。</p><p><b>② 慣らし</b>(2 つの道から一つ):</p><ul><li><b>AI スマート慣らし</b>:開始して自動停止まで任せる(「最適状態まで慣らし完了」);その後<b>慣らしガイド</b>のバナーでもう 1 回やるか判断(「ゴールデンポイント到達」で完了)。</li><li><b>自分の配合</b>:「モーター慣らし」(電圧／回転数モード)で慣れた配合を実行——前後測定は同じく対照でき、<b>あなたの配合がどれだけ効果を出したか、データが教えます</b>。</li></ul><p><b>③ 慣らし後測定</b>:再度「AI トルク予測」を実行し、前と対照:</p><table class='manual-table'><thead><tr><th>指標</th><th>慣らしが効いた様子</th><th>解釈</th></tr></thead><tbody><tr><td><b>慣らし前／後の回転数(慣らしページ表示)</b></td><td><b>上昇</b></td><td><b>主判読指標(全モーター共通)</b>——同電圧で回転数が上がる＝接触改善の最も直接的な現れ</td></tr><tr><td>内耗％</td><td>使用済み／汚れ／長期保管:<b>明確に低下</b>;新品:横ばいまたは微増は正常</td><td><b>使用済みモーターの主判読</b>——内部摩擦と損失が減った＝本当に開いた;低下型は下がりきって止まる＝慣れ切った(新品の判読は下の新品注記)</td></tr><tr><td>トルク(@2.4V／3.0V)</td><td>横ばいまたは微増</td><td>トルクは主に磁石で決まり慣らしで大きく変わらない;明確に低下は異常、点検を</td></tr><tr><td>素性％</td><td>上昇も低下もあり得る(±5 以内＝横ばい)</td><td>総合スコア;慣らし前測定は不確かさが大きく(ブラシ未貼合)、<b>前後差 ±5 以内はすべて測定ばらつき</b>——慣らし効果の判読は回転数を主に</td></tr></tbody></table><div class='manual-note'><b>新品の注記</b>:新品未慣らしのモーターは、内耗％の<b>起点の高低が個体とオイル状態で異なり</b>——慣らし後に<b>明確に低下</b>するものも、元来クリーンで低抵抗で慣らし後<b>横ばい・微増</b>のものもあり(ブラシ接触面積の増加、初生カーボン)、<b>どちらも正常</b>。したがって新品は<b>内耗％単独で慣らし効果を判読しない</b>——主に<b>慣らし指標曲線の低下</b>と<b>慣らし前後の回転数上昇</b>を見てください;「内耗が明確に低下＝開いた」の判読は主に<b>使用済み・汚れた・長期保管のモーター</b>に当てはまります。</div><div class='manual-note'><b>Pro なし?</b>「モーター特性測定」(全版で可)で前後対照も可——主に <b>★I0(無負荷電流)と T_loss が下がるか</b>を見る;新品の判読は上の新品注記と同じ。</div><p><b>④(推奨)健康指紋を作成</b>:慣らし完了＝この個体のピーク状態——この時「AI スマートモーター健康管理」で<b>健康指紋</b>を作れば、以後の定期検査でピークからどれだけ離れたか把握できます(「十二、AI 健康管理」参照)。</p><div class='manual-note'><b>測定温度条件(v3.7.0 から自動処理)</b>:システムは各測定前に<b>同一の測定基準温度まで自動ウォームアップ</b>し、慣らし前／後の温度条件は自動的に一致——手動で待つ必要はありません;結果ページの「測定温度」で確認できます。慣らし直後で機器がまだ熱い場合は、数分待ってから慣らし後測定を(温度が基準付近に戻ればシステムが自動で引き継ぎます)。</div>"
            )},
            {"id": "torque", "t": "八、AI トルク予測システム(Pro)", "html": (
                "<p class='manual-intro'>約 1~2 分の測定で、<b>タミヤ公式規格を基準</b>にこのモーターのトルクと素性スコアを推算します。「この個体は何点か」に答えます——<b>公式標準と比べ、他の個体とは比べません</b>。</p><p><b>操作</b>:ホーム →「AI トルク予測システム」→ <b>型番</b>選択(タミヤ全 15 種;型番を正しく選ばないと基準がずれます)+ <b>備考</b>(任意、40 字以内)→「予測開始」→ 全自動連続測定(約 1~2 分、途中でモーターに触れない)→ 結果ページを表示。</p><p><b>結果ページの見方</b>:</p><table class='manual-table'><thead><tr><th>項目</th><th>意味</th><th>判読</th></tr></thead><tbody><tr><td><b>予測トルク @2.4V／@3.0V</b></td><td>2 つの常用電圧での推算トルク(mN·m)</td><td>下の「タミヤ標準」と対照し、どの帯に入るか</td></tr><tr><td><b>素性 vs タミヤ標準</b></td><td>公式標準に対するトルクの割合(%)</td><td><b>約 100%＝素性は正常</b>;これは生まれつきで慣らしでは変わりません</td></tr><tr><td><b>真値電流 @2.4V／@3.0V</b></td><td>公式負荷条件へ換算した等価電流(A)</td><td>「タミヤ標準」電流帯と対照——「公式条件で回すとどれだけ食うか」</td></tr><tr><td><b>モーター内耗</b></td><td>空転自己消費、<b>低いほど良い</b></td><td>判読は「二十、物理知識」の「モーター内耗」条(3 状態)</td></tr><tr><td><b>測定一致性チェック</b></td><td>今回の 3 回測定が安定か</td><td>「合格」表示のみ採用;「不合格」は取付／センシング不安定、再測定</td></tr><tr><td><b>状態判読行(結果ページ最下部)</b></td><td>機器が直接示す平易な結論</td><td>✅ 緑＝内部損失は正常、安心して使用可;⚠ 琥珀＝損失が高め(先に慣らし／清掃)または読み値が異常に低い(導電オイルの疑い、洗浄して再測定)。<b>結論を分数より優先</b></td></tr></tbody></table><p><b>必ず知っておくこと</b>:</p><ul><li><b>これは「参考値」、精度はトルク ±3%、素性スコア ±5%</b>——どの帯か・同一個体の前後変化を見て、<b>単点の小数にこだわらない</b>。</li><li><b>タミヤ公式標準が基準</b>(実測トルク計の絶対値ではない);公式規格はプレイヤー共通の座標なので、スコアで会話しやすくなります。</li><li><b>慣らし後の素性スコアが新品時より少し低い＝正常</b>(新品は慣らし後にブラシ接触面積が増え、空転自己消費が微増)——劣化ではありません;慣らし効果は「七、標準慣らしフロー」の判読表を参照。</li><li><b>同型番の別個体でスコアが違う＝正常</b>:個体差の反映で測定ミスではありません。</li><li><b>型番は必ず正しく</b>:誤った型番＝誤った基準で比較＝スコアが無意味に。</li></ul><div class='manual-note'><b>注意</b>:全自動・モーターに触れない;「一致性チェック不合格」時は取付と回転センサー位置合わせを確認して再測定;高温時は自動中止。</div>"
            )},
            {"id": "test", "t": "九、モーターテスト", "html": (
                "<p class='manual-intro'>単段階のリアルタイム観察、<b>履歴に保存しません</b>。</p><p>ホーム →「モーターテストプログラム」→ <b>電圧(0.6~4.0V、既定 1V)/ 運転時間(10~600 秒、既定 60)/ 方向 / 安定電流公差(2~50mA)/ 判定基準</b>(純時間制限 または スマート安定＝電流が早めに安定したら早期終了、既定は後者;この設定は慣らしページと<b>独立</b>)を設定 →「開始」→ リアルタイムデータと図表(<b>KV</b> 即時値含む)を見る、時間到達または安定で自動停止。</p><div class='manual-note'><b>注意</b>:上限 4.0V はモーター保護、迂回しない;逆転から正転へ切替前に一度「停止」を。機器が測定基準温度に達していない場合、システムが自動で先にウォームアップしてから開始(計時にウォームアップは含まない);環境温度が低すぎ／高すぎる場合は「データは参考のみ」と表示。</div>"
            )},
            ...
            {"id": "health", "t": "十二、AI スマートモーター健康管理(Pro)", "html": (
                "<p class='manual-intro'>各モーターに<b>健康指紋</b>——慣らし完了時のピーク状態のスナップショット——を作り、以後定期的に再測定して比較、機器がこの個体の「維持具合・どこから劣化し始めたか」を直接教えます。<b>自分と比べ、他個体とは比べません</b>。</p><p><b>正しい使用順序(重要)</b>:先にモーターを<b>慣らし完了</b>(AI スマート慣らしで自動停止まで推奨)させてから指紋を作成。指紋はこの個体の最良状態を表し、以後の各検査は「ピークからどれだけ離れたか」を問います。</p><ol><li>ホーム →「AI スマートモーター健康管理」→「+ モーター追加」。</li><li><b>型番</b>(プルダウン、必須——診断にはその型番の標準が必要)+ <b>備考</b>(任意)を入力。</li><li>確認 → 約 <b>4.5 分</b>で指紋を作成(特性測定 3 回 → ブラシ接触 → 整流リップル記録)。「<b>まだ慣らし切れていない可能性</b>」と出たら:この個体の内部損失がまだ高い——先に AI スマート慣らしを実行し、完了後に指紋を再作成。</li></ol><p>以後、各カードで:</p><ul><li><b>フル検査</b>(約 4.5 分):<b>磁力／内部損失／ブラシ接触</b>の 3 面を指紋と比較、最も完全。</li><li><b>クイック検査</b>(約 1.5 分):内部損失のみ比較、日常の速篩に。</li></ul><p>結果ページにスコア(0~100、「悪い方向」の変化のみ減点)、等級(Optimal / Acceptable / Warning / Critical)、トレンド図と平易なアドバイスを表示。温度差は自動補償——冬に作った指紋を夏に検査しても誤判定しません。</p><p><b>指紋を再作成する時</b>:モーター洗浄／深いメンテ後、または検査で「指紋より滑らか」と出た時——新状態を新基準に。旧指紋は履歴に残り、このモーターの生涯履歴になります。</p><div class='manual-note'><b>注意</b>:各個体最大 50 回履歴、機器全体で最大 20 個;検査中は電源を切らない／他ボタンを押さない;高温時は自動中止;旧版で作成した指紋は新診断の利用に一度の再作成が必要;M1 版はボタングレー(タップで升級案内)。</div>"
            )},
            {"id": "bearing", "t": "十三、ベアリング抵抗テスト", "html": (
                "<p class='manual-intro'>ベアリングの滑らかさを測定、どの版でも可。<b>完全停止までの時間が長いほど → ベアリングが滑らか</b>。</p><p>ホーム →「ベアリング抵抗テスト」→ テスト電圧選択(<b>2.4V または 3V</b>)→「テスト開始」→ 機器がその電圧まで加速 → 安定回転 5 秒 → <b>直接電源断(ブレーキなし)</b> → 完全停止まで計時 → <b>完全停止時間</b>を表示。</p><div class='manual-note'><b>注意</b>:無負荷慣性が小さいため、本テストは<b>時間のみ表示・良否評定なし</b>、同一個体の時期別や別個体の時間を互いに比較してください;回転センサー正常が必要;再現性は 2~3 回。温度が低いとオイルが粘く時間が短めに出ます——システムは毎回<b>測定基準温度まで自動ウォームアップしてから測定</b>し、時期や個体をまたいで結果を比較できます。</div>"
            )},
            {"id": "brush", "t": "十四、ブラシ接触安定テスト", "html": (
                "<p class='manual-intro'>ブラシ接触が均一かを測定、約 35 秒、どの版でも可。<b>接触不均一 → 電流の揺れ → CV 上昇</b>。</p><p>ホーム →「ブラシ接触安定テスト」→「テスト開始」でよい(<b>軸を手で回す必要なし</b>)。流れは <b>1V</b> 固定:電圧を 10 秒で 0→1V へ緩上昇 → 1V を 5 秒保持 → 1V で電流を 20 秒サンプリング → <b>電流変動係数 CV%</b> を表示。</p><p><b>CV% の見方</b>:CV は固定電圧下の電流の揺れ、<b>相対比較ツールで絶対的な良否線はなし</b>(型番ごとに元来違う)。使い方:</p><ul><li>同一個体 <b>慣らし前 vs 慣らし後</b>:CV 低下 = ブラシが馴染み接触改善。</li><li>同一個体 <b>長期追跡</b>:CV 上昇 = ブラシ摩耗 / 整流子の汚れ。</li><li><b>モーター A vs B</b> の相対比較。</li></ul><div class='manual-note'><b>注意</b>:1V 固定・流れ固定でこそ、回をまたぎ・個体をまたいで公平比較可;再現性は 2~3 回。</div>"
            )},
