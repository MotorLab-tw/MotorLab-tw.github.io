...
MANUAL = {
    ...
    "i18n": {
        "zh": {
            ...
            "description": "MotorLab Mini 4WD® 馬達磨合機完整使用手冊 — 連線、馬達特性量測、磨合兩種模式、AI 智慧磨合、馬達測試、歷史紀錄、全球磨合資料庫、AI 健康管理、軸承/電刷測試、田宮馬達規格、系統設定、安全保護、WiFi 重設與常見問題。對應韌體 v3.5.1。",
            "keywords": "MotorLab 使用手冊, 馬達磨合機操作, 馬達磨合機說明書, MotorLab 教學, 馬達磨合機設定",
            "breadcrumb": "使用者手冊",
            "eyebrow": "Docs · 使用者手冊",
            "h1": "MotorLab 使用者手冊",
            "lead": "Mini 4WD® 馬達磨合機完整操作說明。對應韌體 v3.5.1。功能隨版本更新,新版可能多出未列選項。",
            "toc_label": "目錄",
            "fw_note": "本手冊適用 MotorLab v3.5.1 韌體(2026-07-23)。",
        },
        "en": {
            "title": "MotorLab User Manual: Motor Break-in Machine Guide | MotorLab",
            "description": "Complete user manual for the MotorLab Mini 4WD® motor break-in machine — connection, motor characterization, two-mode break-in, AI smart break-in, testing, history records, global database, AI health management, bearing/brush tests, Tamiya motor specs, system settings, safety, WiFi reset and FAQ. For firmware v3.5.1.",
            ...
            "toc_label": "Contents",
            "fw_note": "This manual applies to MotorLab firmware v3.5.1 (2026-07-23).",
        },
        "ja": {
            "title": "MotorLab ユーザーマニュアル:モーター慣らし機操作ガイド | MotorLab",
            "description": "MotorLab Mini 4WD® モーター慣らし機の完全ユーザーマニュアル — 接続、モーター特性測定、慣らし 2 モード、AI スマート慣らし、テスト、履歴記録、グローバルデータ庫、AI 健康管理、ベアリング/ブラシ測定、タミヤモーター規格、システム設定、安全保護、WiFi リセット、FAQ。ファームウェア v3.5.1 対応。",
            ...
            "toc_label": "目次",
            "fw_note": "本マニュアルは MotorLab ファームウェア v3.5.1（2026-07-23）に対応します。",
        },
    },
    "sections": {
        "zh": [
            {"id": "precautions", "t": "〇、使用前必讀・安全須知", "html": (
                ...
                "<p class='manual-intro'>本機內建高溫保護、過流／短路／卡轉／離治具偵測、緩啟動緩停止等多重自動防護(詳見「十五、安全與保護」),但這些是輔助,<b>不能取代使用者的安全操作與看管</b>。</p>"
            )},
            {"id": "start", "t": "一、快速開始", "html": (
                "<div class='manual-note'>🔴 <b>接電源前務必確認</b>:本機須用 <b>DC 5V／6A 以上</b>、具安規認證的電源供應器。<b>使用規格不符的電源會導致機器損壞、甚至引發火災,且不予保固</b>(詳見「〇、使用前必讀」與「十五、安全與保護」)。</div>"
            ...
            {"id": "health", "t": "十、AI 智慧馬達健康管理(Pro)", "html": (
        ...
        "en": [
            ...
            {"id": "home", "t": "3. Home Screen", "html": (
                "<p>Twelve function buttons:</p><p class='manual-pills'>Motor Characterization · Break-in · Motor Test · History · AI Motor Health (Pro) · AI Torque Prediction (Pro) · AI Smart Break-In (Pro) · Bearing Resistance · Brush Stability · Global Break-in Database · Tamiya Motor Specs · System Settings</p><p>The title shows <code>MotorLab M1</code> or <code>MotorLab PRO</code>.</p>"
            )},
            ...
            {"id": "breakin", "t": "5. Break-in (Two Modes)", "html": (
                "<p class='manual-intro'>Long, low-speed running that beds the brushes to the commutator. The top of the break-in page has <b>two mode buttons</b> (cannot switch while running).</p><table class='manual-table'><thead><tr><th>Mode</th><th>Best for</th><th>In one line</th></tr></thead><tbody><tr><td><b>Voltage mode</b></td><td>Players with their own recipe</td><td>Fixed voltage per stage (the classic way)</td></tr><tr><td><b>Speed mode Beta</b></td><td>Players who prefer an RPM target</td><td>Auto speed-control to your target; advances when voltage stabilizes</td></tr></tbody></table><p>Want <b>one tap, fully automatic, stops itself when done</b>? Use the standalone home-screen feature <b>AI Smart Break-In</b> (Pro — see “6. AI Smart Break-In”).</p><h3>Voltage mode</h3><p>Home → “Break-in Program” → pick <b>motor model</b> (16) + <b>note</b> (up to 40 chars) → pick <b>stop criterion</b> (Time-only = each stage runs its full time; Smart Stable = advance early once current settles, time is the cap) → pick <b>passes</b> (1–6, default 1) → confirm the 10-stage parameters for each pass (use the tabs to switch passes; click a number cell to edit) → “Start.”</p><ul><li>Each stage: voltage (0.6–4.0V, <b>0 = skip</b>) / direction / run seconds (10–600, default 60) / cooling seconds (10–600, default 60) / stable-current tolerance (2–50mA).</li><li>With multiple passes, “<b>copy this pass to all following</b>” fills the table fast; the <b>estimated total time</b> shows next to the pass count.</li><li>Out-of-range input is auto-clamped to the limit with a single flash.</li></ul><div class='manual-note'><b>Tip</b>: on a fresh motor the brushes and commutator are not yet formed — tighten the stability tolerance gradually from loose to strict across passes.</div><h3>Speed mode Beta</h3><p>Same 10-stage table as voltage mode, but the first column becomes <b>target RPM</b> (6000–30000, steps of 1000, <b>0 = skip</b>) — the machine adjusts voltage to lock the motor at that speed.</p><ul><li><b>Stop criterion</b> (Strict / Standard / Loose): once the drive voltage at that speed is stable, the stage is considered broken-in and advances early; the run seconds act as an upper bound. Strict = needs to be steadier (deeper, longer); Loose = advances faster (switch to it if a brand-new motor never advances).</li><li>The stage result column shows the <b>stable voltage</b>; if the motor can’t reach the target RPM it runs the full time at the safe voltage cap and is flagged <b>“not reached”</b> (usually the target is set too high, or the motor / jig is faulty).</li><li>Direction, time, passes and copy all work the same as voltage mode.</li></ul><h3>Common behavior</h3><p>Each stage: soft-start → running → soft-stop → cooling → next stage; after a pass it auto-continues to the next (shows “pass x/y · stage c”). All done → 3 beeps → auto-saved to History (voltage / speed mode, with every pass and stage result).</p><p><b>While running you can press</b>:</p><table class='manual-table'><thead><tr><th>Button</th><th>Action</th></tr></thead><tbody><tr><td>Stop</td><td>Soft-stops and ends; record marked “user aborted”</td></tr><tr><td>Reset</td><td>Clears max-RPM / stable-current peaks without interrupting</td></tr><tr><td>Home</td><td>Returns home; break-in <b>keeps running in the background</b></td></tr></tbody></table><div class='manual-note'><b>Note</b>: all settings lock while running; don’t cut power mid-run (data loss); overheating auto-stops and saves; to bench-check quickly, shorten each stage’s time. One extra live field, <b>KV (rpm/V)</b> = current speed ÷ motor voltage, for a quick read on motor state.</div><div class='manual-note'><b>Important: do not use “conductive break-in oil” when measuring / breaking in.</b> Some break-in oils marketed as “conductive” (they contain conductive carbon powder) make RPM jump and voltage drop the instant you add them — that’s <b>not real break-in, just an illusion from the carbon temporarily conducting</b>. Every judgment here (stable current / RPM / internal resistance) measures the <b>true brush-commutator contact</b>; conductive oil skews readings optimistically and reverts once washed off. Correct approach: don’t use such oils at all; if you already did, wash it off thoroughly before measuring. Normal light bearing lubrication is unaffected — this warning is only about “conductive break-in oil.”</div>"
            )},
            {"id": "aibreakin", "t": "6. AI Smart Break-In (Pro)", "html": (
                "<p class='manual-intro'>One-tap fully automatic break-in: the system grinds, measures and stops itself when done — with a built-in <b>Break-in Guide</b> that reads across sessions to answer the hardest question of all: “<b>how many sessions does THIS motor actually need?</b>”</p><p><b>Operation</b>: Home → “AI Smart Break-In” → pick <b>motor model</b> (all 15 Tamiya motors) + <b>note</b> (optional, up to 40 chars) + <b>time cap</b> (until done / 2h / 3h / 4h) → “Start.” The rest is automatic — the system breaks in at progressive speeds while measuring continuously, and <b>stops automatically on detecting steady state (no further gain this session)</b> with a beep.</p><ul><li>The live data and charts on top are <b>identical to the break-in page</b>; below: current round / current action / elapsed time / convergence progress, plus the <b>break-in metric curve</b> (per-round points + moving average; a flattening curve = this session is nearly done).</li><li>The motor <b>alternates forward/reverse</b> (bedding both brush faces symmetrically) and measures periodically; rhythmic changes in sound and speed are normal.</li><li>“Stop” <b>stops immediately</b>; data from completed rounds is kept. Manually stopped sessions appear in the Guide history but are <b>excluded from judgment</b> (marked ✗).</li></ul><h3>The Break-in Guide — answering “how many sessions”</h3><p><b>One session done = motor done? Not necessarily.</b> Each “session” stops at steady state, but a new motor usually needs <b>several consecutive sessions</b> to truly settle; grinding past that point gains nothing and starts consuming brush and magnet life (see “demagnetization” and “break-in sweet spot” in “18. Physics Basics”). The Guide’s job is to catch <b>the session where you should stop</b>.</p><p><b>How it works</b>: after every session the system records its convergence result; when the next one finishes it compares against all previous sessions — still clearly dropping = still improving; flat in a row = gains exhausted = <b>the break-in sweet spot</b>. The verdict shows right on the panel banner:</p><table class='manual-table'><thead><tr><th>Banner</th><th>Meaning</th><th>What to do</th></tr></thead><tbody><tr><td>Guide observing — one more session to start judging</td><td>Not enough sessions yet</td><td>Keep going normally</td></tr><tr><td>Still improving — another session is worth it (x mA saved vs start)</td><td>There is still headroom</td><td><b>Run one more session</b></td></tr><tr><td>Sweet spot reached — stop recommended, little left to gain</td><td>Gains are exhausted; now is the time</td><td><b>Stop; this motor is done</b></td></tr><tr><td>Past the point — you are now grinding lifespan, stop immediately</td><td>The sweet spot was a few sessions ago</td><td>Stop now; don’t break this one in again</td></tr></tbody></table><p><b>When to look, what to press</b>:</p><ol><li><b>First session on a new motor</b>: you will only see “observing” — the first session builds this motor’s baseline; normal.</li><li><b>After each later session, glance at the banner</b>: “still improving” → run another; “<b>sweet spot reached</b>” → stop, this motor’s break-in is complete.</li><li><b>The “Guide history” prompt at start</b> (appears only when history exists): continuing the <b>same motor</b> → choose “continue same motor” (default); a <b>different motor</b> mounted → choose “<b>new motor (start over)</b>” so the Guide record resets — without a reset it compares against another motor’s history and misjudges.</li><li><b>Forgot to pick “new motor”?</b> The system cross-checks the numbers and pops an amber warning when they clearly don’t match this motor’s history; you can also reset manually with “clear Guide history” under the history table (confirmation dialog first; cannot be undone).</li></ol><p><b>Reading the Guide history</b>: each row = one session (newest on top) with time, model, rounds and that session’s convergence value. <b>Read bottom-up: values falling = still improving; falling then flat = the sweet spot</b> — this table is exactly what the banner verdict is based on, so you can verify it with your own eyes. Rows marked ✗ (fault or manual stop) are excluded; ignore them.</p><div class='manual-note'><b>Note</b>: the Guide judges per <b>model</b> — switching models starts fresh automatically; but <b>different units of the same model</b> must be separated by choosing “new motor” at start. Every motor’s sweet spot lands on a different session count (condition and batch vary) — <b>trust the Guide’s verdict, don’t memorize session counts</b>. Judgment is based on same-condition measurements on this machine; the first session after changing the jig or a large ambient-temperature swing may be offset — run one extra session before judging.</div>"
            )},
            {"id": "test", "t": "7. Motor Test", "html": (
            ...
            {"id": "database", "t": "9. Global Break-in Database", "html": (
            ...
            {"id": "health", "t": "10. AI Motor Health (Pro)", "html": (
            ...
            {"id": "bearing", "t": "11. Bearing Resistance Test", "html": (
            ...
            {"id": "brush", "t": "12. Brush Contact Stability Test", "html": (
            ...
            {"id": "tamiya", "t": "13. Tamiya Motor Specs", "html": (
            ...
            {"id": "faq", "t": "17. FAQ", "html": (
                "<table class='manual-table'><thead><tr><th>Problem</th><th>Fix</th></tr></thead><tbody><tr><td>No beep on power-up</td><td>Check the 5V supply</td></tr><tr><td>Connects but page won’t open</td><td>Confirm <code>http://</code> (not https), turn off mobile data, hard-refresh (Ctrl+Shift+R)</td></tr><tr><td>Stuck on “Applying…”</td><td>Usually unstable WiFi — just refresh the page (a built-in 8-second monitor turns the text red)</td></tr><tr><td>Phone / tablet disconnects during a long break-in</td><td>Idle devices auto-drop the hotspot — this is normal; reconnect to the machine’s WiFi and refresh the page. The break-in keeps running on the machine, unaffected by the disconnect</td></tr><tr><td>Forgot the WiFi password, can’t connect</td><td>Hold the on-board WiFi-reset contact for 5 s (see “16. WiFi Reset”); it only resets the hotspot name and password — all other settings and records are kept</td></tr><tr><td>It stopped with 4 long beeps — how do I recover?</td><td>Clear the cause shown on screen (motor not clamped / poor terminal contact / output short / sensor misaligned) → no reboot needed, just press “Start” again; the fault state clears automatically</td></tr><tr><td>It auto-stops a few seconds after starting</td><td>Usually a protection action — check the on-screen reason: current too low = motor not connected; current too high = stalled or shorted; also check for a red under-supply warning</td></tr><tr><td>Motor spins but speed reads 0</td><td>Check that the reflective mark faces the sensor and the jig is aligned; avoid strong light on the sensor area. Persistent no-reading triggers a protective auto-stop (~4 s)</td></tr><tr><td>Overheat lock won’t clear</td><td>Wait for the temperature to drop below the threshold, then press “Reset”; if ambient is high, raise the threshold in System Settings (max 60°C) or check the temperature-calibration offset</td></tr><tr><td>Speed-mode stage flagged “not reached”</td><td>Target RPM exceeds the motor’s ability (it finishes the stage at the safe voltage ceiling) → lower it; if the motor is clearly strong, check the jig mounting and sensor</td></tr><tr><td>Why does AI Smart Break-In sometimes run so long?</td><td>It stops only when it “detects a steady state,” and motors vary a lot; the time cap (2h / 3h / 4h) is the backstop</td></tr><tr><td>Is one AI Smart Break-In session enough?</td><td>One session = one chapter, not necessarily the whole story; watch the <b>Break-in Guide</b> banner — “still improving” means run another, “<b>sweet spot reached</b>” is the sign to stop (see “6. AI Smart Break-In”)</td></tr><tr><td>The Guide keeps showing “observing”</td><td>Not enough sessions yet (the first session is always “observing” — it builds the baseline); manually stopped or faulted sessions (marked ✗ in history) are excluded — run one more full session</td></tr><tr><td>Swapped motors but forgot to pick “new motor”</td><td>The Guide pops an amber warning when the numbers clearly don’t match this motor’s history; press “clear Guide history” under the history table to reset and start over</td></tr><tr><td>Which motors are supported?</td><td>Tamiya Mini 4WD 130-type motors (single- or double-shaft, both fit the jig)</td></tr><tr><td>Power-supply requirements?</td><td>DC 5V / 6A or more with safety certification; when supply is insufficient the UI shows a warning, common with thin cables or low-wattage adapters. <b>A non-compliant supply can damage the machine and poses a fire risk, and is not covered by warranty</b> (see “15. Safety &amp; Protection”)</td></tr><tr><td>Get License stuck connecting</td><td>Wrong home-WiFi password / no internet / weak signal → re-enter or switch WiFi</td></tr><tr><td>After a Pro refund</td><td>The machine <b>auto-reverts to M1</b> on the next online re-check; press “Get License” to buy again</td></tr><tr><td>Red “AP password still default” banner</td><td>Change the password in WiFi settings</td></tr><tr><td>Is my measurement data uploaded to the cloud?</td><td>No — it’s 100% stored on the machine; only recipes you actively “upload” go to the global database</td></tr><tr><td>What happens when history hits 50 records?</td><td>By default it auto-overwrites the oldest (can be turned off); export important records as JSON/CSV first</td></tr><tr><td>Will a software update wipe my records?</td><td>No — updates keep all settings, calibration and history</td></tr><tr><td>Can a failed update brick it?</td><td>No — there’s automatic rollback to the previous version</td></tr><tr><td>Can I unplug at idle?</td><td>Yes; best when the motor is fully stopped, no “Applying…,” no update in progress</td></tr></tbody></table>"
            )},
            {"id": "physics", "t": "18. Physics Basics", "html": (
                "<p class='manual-intro'>Plain-language explanations of the physics terms you meet while measuring and breaking in — so the numbers mean something.</p><table class='manual-table'><thead><tr><th>Term</th><th>In plain words</th></tr></thead><tbody><tr><td><b>Steady state (convergence)</b></td><td>The state where the break-in metric <b>stops improving</b> = done. There is no fixed number of passes; what matters is whether the metric has converged. <b>AI Smart Break-In</b> watches for this continuously and auto-stops the session at steady state. Voltage / speed modes only judge whether a <b>single stage</b> is stable (shown in the stage status column); overall progress is yours to read across passes. Tip: on a new motor, tighten the stability tolerance <b>from loose to strict across passes</b> — a loose early tolerance lets the rough phase advance, a strict final pass certifies “truly stable”</td></tr><tr><td><b>Break-in sweet spot</b></td><td>The ideal stopping point where a motor’s gains are just used up and wear has not yet begun. Break-in gains are finite: each early session visibly improves, then one session the gains hit zero — beyond that you only wear brushes and demagnetize (see “Demagnetization”), giving back what you earned. The sweet spot differs per motor and cannot be memorized as a session count — the <b>Break-in Guide</b> (see “6. AI Smart Break-In”) catches it by comparing data across sessions: flat in a row = the spot, and the banner says “stop recommended”</td></tr><tr><td><b>Stop criterion</b></td><td>The rule for “what counts as stable, when to move on” in each break-in stage (selectable on the break-in page). “<b>Smart Stable</b>” judges stability automatically so you don’t estimate times yourself</td></tr><tr><td><b>Stage advance</b></td><td>Moving from one break-in stage to the next. When the stop criterion is met the stage advances automatically — how fast stages advance is itself a signal of progress</td></tr><tr><td><b>Stable voltage</b></td><td>The motor’s actual voltage at the moment a stage is judged stable. Comparing it across passes under the same conditions is quantified evidence of break-in progress</td></tr><tr><td><b>Stability tolerance</b></td><td>The allowed fluctuation band for “stable enough” — the smaller the tolerance, the stricter the gate (see “Steady state” for setting tips)</td></tr><tr><td><b>Start-up voltage</b></td><td>The minimum voltage that gets the motor turning from rest. Lower = smoother internals</td></tr><tr><td><b>Commutation spark</b></td><td>The tiny arc as the brushes switch current on the commutator. It is at once noise, loss and the driver of magnet decay — the smoother the contact, the smaller the spark</td></tr><tr><td><b>Demagnetization</b></td><td>Magnets getting weaker. Magnetism is not “used up” (a shelf magnet lasts a century) — it is <b>knocked off</b>: every commutation spark is a tiny reverse-field hit; hours at high RPM (especially with reversals) add up to hundreds of millions of hits, and the magnets weaken <b>irreversibly</b>. RPM then looks faster — an illusion of weaker magnets, not a stronger motor. <b>Stop at the sweet spot</b> (the AI Smart Break-In Guide tells you) — every extra minute is lifespan spent</td></tr><tr><td><b>False conduction (conductive-oil illusion)</b></td><td>Carbon powder temporarily conducting makes numbers look great — an <b>illusion</b>, not real break-in. This machine judges from the motor’s true electrical state; conductive oil skews readings optimistically — wash it off before measuring (see the warning in “5. Break-in”)</td></tr><tr><td><b>KV</b></td><td>Current RPM ÷ the motor’s actual voltage (rpm/V) — a quick state indicator: higher at the same voltage means faster</td></tr><tr><td><b>Motor internal loss (no-load loss)</b></td><td>The power a motor burns just spinning itself (brush friction + bearing drag) — think of an engine’s <b>idle fuel burn</b>: consumed before the car moves. <b>Lower is better</b>: low internal loss = more power actually pushes the car — stronger re-acceleration out of corners, longer runtime per battery, cooler motor, less fade. <b>The percentage baseline</b> = each model’s “standard internal loss” derived from official specs is 100% (e.g. 30% means this unit idles at about a third of the standard baseline — one ruler across models). <b>What counts as good — read the three phases, not one point</b>: ① <b>still falling</b> = break-in still improving (e.g. new 30% → done 26%); ② <b>falls then flattens</b> = this unit’s floor = done (brushed Dash-series field reference: new around 30%, settled about 25–27%); ③ <b>rebounds off the floor</b> = brush / commutation aging or over-break-in warning. It is not “lower forever is better” — the floor is set by construction; <b>an abnormally low reading should first make you suspect measurement or false conduction</b> (see “False conduction”) — re-test to confirm</td></tr><tr><td><b>Constitution (% of official spec)</b></td><td>One output of AI torque prediction: this unit’s innate torque as a percentage of Tamiya’s official standard — about 100% is normal. It reflects the winding and magnets it was born with — <b>break-in cannot change constitution</b> (strength is innate; break-in changes how much power is wasted)</td></tr><tr><td><b>Efficiency score</b></td><td>AI torque prediction’s composite score: merges “output” and “power draw” into one cross-unit efficiency number — higher means the same force for less power</td></tr><tr><td><b>True-value current</b></td><td>The no-load measurement <b>converted to standard load conditions</b>, for comparison against Tamiya’s official current band. Plainly: “if this unit ran under official conditions, this is roughly what it would draw”</td></tr><tr><td><b>Reference RPM</b></td><td>The baseline RPM Smart Break-In measures every round under <b>fixed identical conditions</b>. With conditions fixed, round-to-round change is purely the motor changing — that is the whole point</td></tr><tr><td><b>Natural drift</b></td><td>The same motor reads slightly different each time — normal. Brush contact and temperature cause small drift; read <b>trends and multi-run averages</b>, not single points</td></tr><tr><td><b>Systematic difference (measurement baseline)</b></td><td>Why your numbers don’t match other people’s: different voltage baselines, supplies, jigs and methods shift everything; <b>before-vs-after on the same machine</b> is the most reliable</td></tr><tr><td><b>Bearing resistance</b></td><td>Mechanical friction between shaft and bearings — one mechanical component of internal loss. Dedicated test: “11. Bearing Resistance Test”</td></tr><tr><td><b>Brush contact stability</b></td><td>How steady the brush-commutator contact is — flickering contact makes readings jump. Dedicated test: “12. Brush Contact Stability Test”</td></tr><tr><td><b>Ripple</b></td><td>The fine, fast fluctuations riding on the current — a “fingerprint” of commutation and contact quality; smoother means better-bedded brushes</td></tr></tbody></table>"
            )},
            {"id": "leds", "t": "19. Status Lights &amp; Sounds", "html": (
                "<p><b>Status lights</b> (default config, editable under “RGB status light”):</p><table class='manual-table'><thead><tr><th>System state</th><th>Default</th><th>Meaning</th></tr></thead><tbody><tr><td>Overheat lock</td><td>Red flashing</td><td>Overheat protection triggered; cool down and press “Reset”</td></tr><tr><td>Cooling</td><td>Yellow solid</td><td>Inter-stage cooling, motor idle</td></tr><tr><td>Idle</td><td>Blue breathing</td><td>Waiting for a command</td></tr><tr><td>Running</td><td>Green breathing</td><td>Break-in / test / check in progress</td></tr><tr><td>Off</td><td>—</td><td>No match or all off</td></tr></tbody></table><p><b>Sound cues</b> (buzzer, not customizable; every cue sounds about 1 s after the event — once the motor has slowed, so each beep is clear):</p><table class='manual-table'><thead><tr><th>Sound</th><th>Situation</th></tr></thead><tbody><tr><td>Race countdown, 4 beeps (3 short + 1 long high)</td><td>Boot complete</td></tr><tr><td>2 beeps</td><td>Motor test complete</td></tr><tr><td>3 beeps</td><td>Break-in complete (voltage / speed mode all passes done; AI Smart Break-In auto-stopped when done)</td></tr><tr><td>5 short beeps</td><td>Overheat lock (cool down, then press “Reset” to clear)</td></tr><tr><td><b>4 long beeps</b></td><td><b>Motor fault stop</b> (no motor / stalled / no speed signal / output short; the screen shows the reason)</td></tr><tr><td>10 beeps</td><td>WiFi reset triggered (auto-restart follows)</td></tr></tbody></table><p class='manual-contact'><b>Reporting an issue?</b> Please include: firmware version (System Settings → Software update), edition (M1 / PRO on home), a screenshot, whether it was running, and repro steps. Support: <b>motorlab.tw@gmail.com</b></p>"
            )},
        ],
        "ja": [
            {"id": "precautions", "t": "〇、使用前に必ずお読みください・安全上の注意", "html": (
                ...
                "<p class='manual-intro'>本機は高温保護、過電流／短絡／ロック／治具外れ検出、ソフトスタート・ソフトストップなど複数の自動保護を内蔵しています(「十五、安全と保護」参照)が、これらは補助であり、<b>安全な操作と監視の代わりにはなりません</b>。</p>"
            )},
            {"id": "start", "t": "一、クイックスタート", "html": (
                "<div class='manual-note'>🔴 <b>電源接続前に必ず確認</b>:本機には <b>DC 5V／6A 以上</b>・安全規格認証済みの電源を使用。<b>規格外の電源は機器を損傷させ、火災の恐れもあり、保証対象外です</b>(「〇、使用前に必ずお読みください」「十五、安全と保護」参照)。</div>"
            ...
            {"id": "home", "t": "三、ホーム画面", "html": (
                "<p>12 個の機能ボタン:</p><p class='manual-pills'>モーター特性測定 · 慣らし · モーターテスト · 履歴 · AI モーター健康管理(Pro)· AI トルク予測システム(Pro)· AI スマート慣らし(Pro)· ベアリング抵抗 · ブラシ安定 · グローバル慣らしデータ庫 · タミヤモーター規格 · システム設定</p><p>タイトルに <code>MotorLab M1</code> または <code>MotorLab PRO</code> と表示されます。</p>"
            )},
            ...
            {"id": "breakin", "t": "五、モーター慣らし(2 モード)", "html": (
                "<p class='manual-intro'>低速で長時間運転し、ブラシと整流子を最適な接触に馴染ませます。慣らしページ上部に<b>2 つのモードボタン</b>(運転中は切替不可)。</p><table class='manual-table'><thead><tr><th>モード</th><th>向く人</th><th>一言</th></tr></thead><tbody><tr><td><b>電圧モード</b></td><td>自分のレシピがある人</td><td>各段階を固定電圧で運転(従来方式)</td></tr><tr><td><b>回転数モード Beta</b></td><td>回転数を基準にしたい人</td><td>各段階で設定回転数に自動制御、電圧が安定すると自動で次段階へ</td></tr></tbody></table><p>「<b>ワンタップ全自動・仕上がったら自動停止</b>」で慣らしたい人は、ホームの独立機能「<b>AI スマート慣らし</b>」をどうぞ(Pro、「六、AI スマート慣らし」参照)。</p><h3>電圧モード</h3><p>ホーム →「慣らしプログラム」→ <b>機種</b>(16)+ <b>備考</b>(40 字以内)→ <b>判停基準</b>(時間のみ=各段階を設定時間いっぱい運転;スマート安定=電流が早く安定したら次段階へ、時間は上限)→ <b>パス数</b>(1~6、初期 1)→ 各パスの 10 段階パラメータを確認(複数パスはタブで切替、数値欄を直接タップして編集)→「開始」。</p><ul><li>各段階の設定:電圧(0.6~4.0V、<b>0=スキップ</b>)/ 方向 / 慣らし秒(10~600、初期 60)/ 冷却秒(10~600、初期 60)/ 安定電流許容差(2~50mA)。</li><li>複数パスでは「<b>このパスを以降すべてにコピー</b>」で素早く入力、パス数の隣に<b>推定総時間</b>を即時表示。</li><li>範囲外の入力は自動的に上下限へ補正し一度点滅して知らせます。</li></ul><div class='manual-note'><b>ポイント</b>:新品モーターはブラシと整流子がまだ成形されていません。パスを追うごとに許容差を大→小へ絞っていくのがコツです。</div><h3>回転数モード Beta</h3><p>電圧モードと同じ段階表ですが、最初の列が<b>目標回転数</b>(6000~30000、1000 刻み、<b>0=スキップ</b>)に変わります——機器が電圧を自動調整してモーターをその回転数にロックします。</p><ul><li><b>判停基準</b>(厳格 / 標準 / 緩め):その回転数での駆動電圧が安定すれば慣れたとみなし早めに次段階へ、慣らし秒は上限。厳格=より安定が必要(深く長く)、緩め=早めに次へ(新品が段階を進めない場合は緩めに)。</li><li>段階結果欄に<b>安定電圧</b>を表示。目標回転数に届かない場合は安全上限電圧で時間いっぱい運転し「<b>未達</b>」と表示(通常は目標が高すぎる、またはモーター / 治具の異常)。</li><li>方向・時間・パス数・コピーの操作は電圧モードと同じです。</li></ul><h3>共通の挙動</h3><p>各段階:ソフトスタート → 運転中 → ソフトストップ → 冷却 → 次段階;1 パス終了で自動的に次パスへ(「x/y パス · 段階 c」を表示)。全完了 → ビープ 3 回 → 履歴に自動保存(電圧 / 回転数モード、各パス各段階の結果を含む)。</p><p><b>運転中に押せるボタン</b>:</p><table class='manual-table'><thead><tr><th>ボタン</th><th>動作</th></tr></thead><tbody><tr><td>停止</td><td>ソフトストップ後に終了、記録に「ユーザー中止」</td></tr><tr><td>ゼロリセット</td><td>最大回転 / 安定電流のピークをクリア(中断しない)</td></tr><tr><td>ホーム</td><td>ホームへ戻る、慣らしは<b>バックグラウンドで継続</b></td></tr></tbody></table><div class='manual-note'><b>注意</b>:運転中は全設定がロック;途中で電源を切らない(データ消失);高温時は自動停止して保存;素早く動作確認したい場合は各段階の時間を短く。リアルタイムデータに <b>KV(rpm/V)</b>=現在回転数 ÷ モーター電圧 の欄が加わり、状態の素早い比較に使えます。</div><div class='manual-note'><b>重要:測定 / 慣らしの際は「導電潤滑オイル」を使用しないでください。</b>「導電」を謳う慣らしオイル(導電カーボン粉入り)を入れると回転数が急上昇・電圧が急低下しますが、それは<b>本当に慣らせたのではなく、カーボンが一時的に導通する錯覚</b>です。本機の全判定(安定電流 / 回転数 / 内部抵抗)はブラシと整流子の<b>真の接触状態</b>を測るため、導電オイルは読値を楽観側に歪め、洗い流すと元に戻ります。正しい方法:この種のオイルは一切使わない;既に使った場合は徹底的に洗浄してから測定を。通常のベアリングへの微量潤滑は影響を受けません —— この警告は「導電型慣らしオイル」のみが対象です。</div>"
            )},
            {"id": "aibreakin", "t": "六、AI スマート慣らし(Pro)", "html": (
                "<p class='manual-intro'>ワンタップ全自動慣らし:システムが自分で回し、自分で測り、仕上がったら自動停止。内蔵の<b>慣らしガイド</b>がセッション横断で判読し、いちばん難しい問い——「<b>このモーター、何セッション回せば十分?</b>」に答えます。</p><p><b>操作</b>:ホーム →「AI スマート慣らし」→ <b>機種</b>(タミヤ全 15 機種)+ <b>備考</b>(任意、40 字以内)+ <b>時間上限</b>(完了まで / 2h / 3h / 4h)→「開始」。あとは全自動——漸進的な回転数で慣らしながら連続測定し、<b>定常状態(本セッションはもう改善しない)を検出すると自動停止</b>してビープで知らせます。</p><ul><li>上部のリアルタイムデータとグラフは<b>慣らしページと完全に同一</b>;下部に現在のラウンド / 現在の動作 / 経過時間 / 収束進捗、および<b>慣らし指標カーブ</b>(各ラウンドの点 + 移動平均、カーブが平坦=本セッション完了間近)。</li><li>途中でモーターは<b>正転・逆転を交互</b>に慣らし(ブラシ両面を対称に馴染ませ)、周期的に自動測定します。音と回転の規則的な変化は正常です。</li><li>「停止」で<b>即時停止</b>、完了済みラウンドのデータは保持。手動停止したセッションはガイド履歴に載りますが<b>判定には不参加</b>(✗ 印)。</li></ul><h3>慣らしガイド——「何セッション?」に答える</h3><p><b>1 セッション完了=仕上がった?とは限りません。</b>自動慣らしは 1「セッション」ごとに定常状態で止まりますが、新品モーターは通常<b>連続数セッション</b>でようやく本当に仕上がります;仕上がった後に回し続けても進歩はなく、ブラシと磁石の寿命を消耗するだけ(「十八、物理の基礎知識」の「減磁」「慣らしのスイートスポット」参照)。ガイドの仕事は「<b>止めるべきそのセッション</b>」を捕まえることです。</p><p><b>仕組み</b>:セッション完了ごとに収束結果を自動記録;次のセッションが終わると過去分と自動比較——まだ明確に下降中=まだ進歩中;連続横ばい=進歩が出切った=<b>慣らしのスイートスポット</b>。判定はパネルのバナーに直接表示:</p><table class='manual-table'><thead><tr><th>バナー表示</th><th>意味</th><th>やること</th></tr></thead><tbody><tr><td>ガイド観察中——もう 1 セッションで判定開始</td><td>セッション数不足、まだ判読不可</td><td>そのまま続ける</td></tr><tr><td>まだ進歩中、続ける価値あり(初期比 x mA 減)</td><td>まだ伸びしろあり</td><td><b>もう 1 セッション回す</b></td></tr><tr><td>スイートスポット到達、停止推奨——これ以上は進歩わずか</td><td>進歩は出切った、今が止めどき</td><td><b>停止、この個体は完成</b></td></tr><tr><td>行き過ぎ——今回しているのは寿命、即時停止を推奨</td><td>スイートスポットは数セッション前</td><td>すぐ止める、以後この個体は回さない</td></tr></tbody></table><p><b>見るタイミング</b>:</p><ol><li><b>新品の最初のセッション</b>:完了後は「観察中」だけ——1 本目はこの個体の基準づくり、正常です。</li><li><b>以降はセッション完了ごとにバナーを一目</b>:「まだ進歩中」なら次を開始;「<b>スイートスポット到達</b>」で停止——この個体の慣らしは完了、そのまま車載へ。</li><li><b>開始時の「ガイド記録」確認</b>(記録がある時のみ表示):<b>同じ個体</b>を続ける→「同じモーターを続行」(既定);<b>別の個体</b>に載せ替えた→「<b>新しいモーター(最初から)</b>」を選択しガイド記録をリセット——リセットしないと別個体の記録と比較して判定が狂います。</li><li><b>「新しいモーター」を選び忘れた?</b>数値がこの個体の履歴と明らかに合わない場合、琥珀色の警告が出ます;履歴表下の「ガイド記録を消去」で手動リセットも可(確認ダイアログあり、消去後は復元不可)。</li></ol><p><b>ガイド履歴の見方</b>:1 行=1 セッション(新しいものが上)、時刻・機種・ラウンド数・そのセッションの収束値を表示。<b>下から上へ:値が下がり続けている=まだ進歩中;下がって横ばい=スイートスポット</b>——この表こそバナー判定の根拠で、自分の目で検証できます。✗ 印の行(故障・手動停止)は判定に不参加、気にしなくて OK。</p><div class='manual-note'><b>注意</b>:ガイドは<b>機種ごと</b>に判読し、機種を替えると自動で仕切り直し;ただし<b>同機種の別個体</b>は、開始時に「新しいモーター」を選ぶことで区別します。スイートスポットのセッション数は個体ごとに違います(新品状態・ロット差の影響)——<b>ガイドの判定を見る、セッション数を暗記しない</b>。判定は本機の同条件測定に基づきます;治具交換や室温の大きな変化直後の 1 本目は値がずれることがあり、もう 1 セッション回してから判読を。</div>"
            )},
            {"id": "test", "t": "七、モーターテスト", "html": (
            ...
            {"id": "database", "t": "九、グローバル慣らしデータ庫", "html": (
            ...
            {"id": "health", "t": "十、AI モーター健康管理(Pro)", "html": (
            ...
            {"id": "bearing", "t": "十一、ベアリング抵抗テスト", "html": (
            ...
            {"id": "faq", "t": "十七、よくある質問", "html": (
                "<table class='manual-table'><thead><tr><th>症状</th><th>対処</th></tr></thead><tbody><tr><td>起動音が鳴らない</td><td>電源 5V を確認</td></tr><tr><td>接続できるがページが開けない</td><td><code>http://</code>(https でない)を確認、モバイルデータをオフ、強制再読み込み(Ctrl+Shift+R)</td></tr><tr><td>「適用中…」のまま</td><td>多くは WiFi 不安定、ページを再読み込み(内蔵の 8 秒監視が赤字で知らせます)</td></tr><tr><td>長い慣らし中にスマホ / タブレットが切断される</td><td>端末はアイドルでホットスポットを自動切断します —— 正常な動作です;機器の WiFi に再接続してページを再読み込み。慣らしは機器側で継続し、切断の影響を受けません</td></tr><tr><td>WiFi パスワードを忘れて接続できない</td><td>機器の WiFi リセット接点を 5 秒長押し(「十六、WiFi リセット」参照)、ホットスポット名とパスワードのみリセットされ、他の設定と記録はすべて保持</td></tr><tr><td>長音 4 回で自動停止、復帰するには</td><td>画面表示の原因を解消(モーター未固定 / 端子接触不良 / 出力短絡 / センサー未整合)→ 再起動不要、そのまま再度「開始」で故障状態は自動クリア</td></tr><tr><td>起動して数秒で自動停止する</td><td>通常は保護動作、画面の原因を確認:電流が低すぎ=モーター未接続;電流が高すぎ=固着または短絡;供給不足の赤い警告がないかも確認</td></tr><tr><td>モーターは回るが回転数が 0 表示</td><td>反射マークがセンサーに向いているか、治具が整合しているか確認;センサー部への強い光を避ける。読めない状態が続くと保護で自動停止(約 4 秒)</td></tr><tr><td>高温ロックが解除できない</td><td>温度が閾値未満に下がってから「ゼロリセット」;室温が高い場合はシステム設定で閾値を上げる(上限 60°C)か温度校正の補償を確認</td></tr><tr><td>回転数モードで段階が「未達」</td><td>目標回転数がモーター能力を超過(安全上限電圧で時間いっぱい運転して終了)→ 目標を下げる;明らかに力があるなら治具の取付とセンサーを確認</td></tr><tr><td>AI スマート慣らしが時々長く回るのはなぜ</td><td>「定常状態を検出」するまで止まらず、モーター差が大きいため;時間上限(2h / 3h / 4h)が保険です</td></tr><tr><td>AI スマート慣らしは 1 セッションで十分?</td><td>1 セッション=1 区切りで、個体全体の仕上がりとは限りません;<b>慣らしガイド</b>のバナーを見て——「まだ進歩中」ならもう 1 本、「<b>スイートスポット到達</b>」が収工の合図(「六、AI スマート慣らし」参照)</td></tr><tr><td>ガイドがずっと「観察中」のまま</td><td>セッション数不足(1 本目は必ず観察中——基準づくり中);手動停止・故障のセッション(履歴 ✗ 印)は判定に不参加、完全に収束したセッションをもう 1 本</td></tr><tr><td>モーターを替えたのに「新しいモーター」を選び忘れた</td><td>数値が履歴と合わないとガイドが琥珀色の警告を表示;履歴表下の「ガイド記録を消去」でリセットしてやり直せば OK</td></tr><tr><td>対応するモーターは</td><td>タミヤ Mini 4WD 130 型モーター(片軸 / 両軸とも治具に装着可)</td></tr><tr><td>電源の要件は</td><td>DC 5V / 6A 以上・安全認証付き;供給不足時は UI に警告、細いケーブルや小容量アダプターで起こりがち。<b>規格外の電源は機器を損傷させ火災の危険があり、保証対象外です</b>(「十五、安全と保護」参照)</td></tr><tr><td>ライセンス取得が接続で止まる</td><td>家庭 WiFi のパスワード誤り / ネットなし / 電波弱 → 再入力または WiFi 変更</td></tr><tr><td>Pro 返金後</td><td>次のオンライン再検証時に<b>自動で M1 へ戻る</b>;再購入は「ライセンス再取得」</td></tr><tr><td>赤い「AP パスワードが初期のまま」バナー</td><td>WiFi 設定でパスワードを変更</td></tr><tr><td>測定データはクラウドにアップロードされる?</td><td>されません。100% 機器内に保存;自分で「アップロード」したレシピのみグローバルデータ庫へ</td></tr><tr><td>履歴が 50 件に達したら</td><td>初期設定では最古を自動上書き(無効化可);重要な記録は先に JSON/CSV でエクスポートを</td></tr><tr><td>ソフトウェア更新で記録は消える?</td><td>消えません。更新は全設定・校正・履歴を保持</td></tr><tr><td>更新失敗で文鎮化する?</td><td>しません。前バージョンへの自動復帰があります</td></tr><tr><td>待機中に電源を抜いてよい?</td><td>可。モーター完全停止・「適用中」なし・更新中でないときが安全</td></tr></tbody></table>"
            )},
            {"id": "physics", "t": "十八、物理の基礎知識", "html": (
                "<p class='manual-intro'>測定と慣らしで出会う物理用語を、平易に分解——数字の意味が読めるように。</p><table class='manual-table'><thead><tr><th>用語</th><th>平たく言うと</th></tr></thead><tbody><tr><td><b>定常状態(収束)</b></td><td>慣らし指標が「<b>もう改善しない</b>」状態=仕上がり。何パス回すかに正解はなく、見るのは指標が収束したか。<b>AI スマート慣らし</b>はこれを常時監視し、定常状態で本セッション完了と判定して自動停止します。電圧 / 回転数モードは「単一段階」の安定のみ判定(段階状態欄に表示)、全体の仕上がりは各パスの結果から自分で判読。コツ:新品は安定許容差を<b>パスごとに大→小へ</b>——序盤は緩めで粗い時期を通し、最終パスの小許容差で「本当に安定」を担保</td></tr><tr><td><b>慣らしのスイートスポット</b></td><td>「進歩をちょうど使い切り、消耗はまだ始まっていない」最良の止めどき。慣らしの進歩は有限:序盤は毎セッション体感できるほど伸び、あるセッションを境に進歩ゼロ——それ以降はブラシ消耗と減磁だけ(「減磁」参照)、稼いだ性能を少しずつ返していきます。スイートスポットは個体ごとに違い、固定のセッション数では覚えられません——<b>慣らしガイド</b>(「六、AI スマート慣らし」参照)がセッション横断のデータ比較で自動的に捕捉:連続横ばい=到達、バナーが「停止推奨」と教えてくれます</td></tr><tr><td><b>判停基準</b></td><td>各慣らし段階の「どこまでが安定か、いつ次へ進むか」の判断ルール(慣らしページで選択)。「<b>スマート安定</b>」は安定度を自動判定、時間を自分で見積もる必要なし</td></tr><tr><td><b>段階送り</b></td><td>慣らしが次の段階へ進むこと。判停基準を満たすと自動で送られる——送りの速さ自体が進捗のシグナル</td></tr><tr><td><b>安定電圧</b></td><td>段階が安定と判定された瞬間のモーター実効電圧。同条件でパス間の変化を比べれば、慣らし進捗の定量的証拠になる</td></tr><tr><td><b>安定許容差</b></td><td>「十分安定」と認める変動幅——小さいほど関門は厳しい(設定のコツは「定常状態」参照)</td></tr><tr><td><b>起動電圧</b></td><td>静止からモーターが回り始める最低電圧。低いほど内部がスムーズ</td></tr><tr><td><b>整流火花</b></td><td>ブラシが整流子上で電流を切り替える瞬間の微小アーク。ノイズ・損失・磁石劣化の源でもある——接触面が馴染むほど火花は小さい</td></tr><tr><td><b>減磁</b></td><td>磁石の磁力低下。磁力は「使って減る」のではなく(棚の磁石は百年もつ)、「<b>叩かれて減る</b>」:整流火花 1 回=逆磁場の微打撃 1 回、高回転を長時間(特に正逆転あり)で累計は億単位、磁力は少しずつ<b>不可逆</b>に弱まる——このとき回転数はむしろ速く見えるが、それは磁力低下の錯覚で、強くなったのではない。<b>スイートスポットで止める</b>(AI スマート慣らしのガイドが教えてくれます)——余分に回す 1 分はすべて寿命の消耗</td></tr><tr><td><b>偽導通(導電オイルの錯覚)</b></td><td>カーボン粉が一時的に導通して数値が良く見える<b>錯覚</b>で、真の慣らしではない。本機の判定はモーターの真の電気状態が根拠;導電オイルは読値を楽観側に歪める——洗浄してから測定を(「五、モーター慣らし」の警告参照)</td></tr><tr><td><b>KV 値</b></td><td>現在回転数 ÷ モーター実効電圧(rpm/V)。状態を素早く比べる指標——同電圧で数値が高いほど速い</td></tr><tr><td><b>モーター内部損失(無負荷損失)</b></td><td>モーターが空転するだけで消費する電力(ブラシ摩擦+ベアリング抵抗)——エンジンの<b>アイドリング燃費</b>のイメージ:車を押す前に消える分。<b>低いほど良い</b>:内部損失が低い=電力がより多く実走に使われる——立ち上がり加速が力強く、同じ電池で長く走り、モーターは涼しく、垂れにくい。<b>パーセントの基準</b>=各機種の公式規格から換算した「標準内部損失」を 100% とする(例 30% はこの個体の空転自己消費が標準基準の約 3 割、機種をまたいで同じ物差し)。<b>良し悪しは 3 つの局面で読む、単点で読まない</b>:①<b>下降継続</b>=慣らしがまだ進歩中(例:新品 30% → 仕上がり 26%);②<b>下がって底で横ばい</b>=この個体の底=仕上がり(カーボンブラシ Dash 系の実測目安:新品 30% 前後、仕上がり 25~27%);③<b>底から反転上昇</b>=ブラシ / 整流面の老化か慣らしすぎの警報。どこまでも低いほど良いのではない——底は構造で決まる;<b>異常に低い時はまず測定か偽導通を疑う</b>(「偽導通」参照)、再測で確認</td></tr><tr><td><b>体質(公式規格比%)</b></td><td>AI トルク予測の出力の一つ:この個体の生まれつきのトルクをタミヤ公式標準に対する百分率で示し、約 100% が正常。巻線と磁石の先天条件を反映——<b>慣らしで体質は変わらない</b>(力は生まれつき、慣らしが変えるのは電気のムダ)</td></tr><tr><td><b>効率スコア</b></td><td>AI トルク予測の総合点:「出力」と「消費」を 1 つに統合した個体間比較可能な効率指標——高いほど、同じ力をより少ない電力で</td></tr><tr><td><b>真値電流</b></td><td>無負荷実測を<b>標準負荷条件に換算</b>した等価電流。タミヤ公式の電流帯と対照するための値。平たく:「公式条件で走らせたら、この個体はだいたい何 A 食うか」</td></tr><tr><td><b>参照回転数</b></td><td>スマート慣らしが毎ラウンド<b>固定の同一条件</b>で測る基準回転数。条件が固定だから、ラウンド間の変化はすべてモーター自身の変化——それがこの値の存在意義</td></tr><tr><td><b>自然ドリフト</b></td><td>同じモーターでも毎回の数値が少し違う——正常。ブラシ接触と温度で小幅にずれる;判読は<b>トレンドと複数回平均</b>で、単発を比べない</td></tr><tr><td><b>系統差(測定基準)</b></td><td>自分のデータが他人 / ネットと合わない理由:電圧基準・供給・治具・測定方法の違いが全体をずらす;<b>同一機器での前後比較</b>が最も信頼できる</td></tr><tr><td><b>ベアリング抵抗</b></td><td>軸と軸受の間の機械摩擦——内部損失の機械成分の一つ。専用テストは「十一、ベアリング抵抗テスト」</td></tr><tr><td><b>ブラシ接触安定</b></td><td>ブラシと整流子の接触の安定度——接触が不安定だと数値が跳ねる。専用テストは「十二、ブラシ接触安定テスト」</td></tr><tr><td><b>リップル</b></td><td>電流に乗る細かく速い揺らぎ——整流と接触品質の「指紋」、滑らかなほどブラシがよく馴染んでいる</td></tr></tbody></table>"
            )},
            {"id": "leds", "t": "十九、ステータスランプと音の合図", "html": (
                "<p><b>ステータスランプ</b>(初期設定、「RGB ステータスライト」で変更可):</p><table class='manual-table'><thead><tr><th>システム状態</th><th>初期</th><th>意味</th></tr></thead><tbody><tr><td>高温ロック</td><td>赤点滅</td><td>高温保護作動、冷却して「ゼロリセット」</td></tr><tr><td>冷却中</td><td>黄点灯</td><td>段階間の冷却、モーター停止</td></tr><tr><td>待機</td><td>青呼吸</td><td>コマンド待ち</td></tr><tr><td>運転中</td><td>緑呼吸</td><td>慣らし / テスト / 検査中</td></tr><tr><td>消灯</td><td>—</td><td>一致なし、または全消灯</td></tr></tbody></table><p><b>音の合図</b>(ブザー、カスタム不可;すべての音はイベント発生の約 1 秒後に鳴ります——モーター減速を待ってから、一音ずつ明瞭に):</p><table class='manual-table'><thead><tr><th>音</th><th>状況</th></tr></thead><tbody><tr><td>レースカウントダウン 4 回(短 3 + 長高音 1)</td><td>起動完了</td></tr><tr><td>2 回</td><td>モーターテスト完了</td></tr><tr><td>3 回</td><td>慣らし完了(電圧 / 回転数モードは全パス終了;AI スマート慣らしは完了で自動停止)</td></tr><tr><td>短 5 回</td><td>高温ロック(冷却後「ゼロリセット」で解除)</td></tr><tr><td><b>長 4 回</b></td><td><b>モーター故障停止</b>(未検出 / 拘束 / 回転信号なし / 出力短絡、画面に理由を表示)</td></tr><tr><td>10 回</td><td>WiFi リセット作動(この後自動再起動)</td></tr></tbody></table><p class='manual-contact'><b>問題報告</b>の際は次を併せてご提供ください:ファームウェアバージョン(システム設定 → ソフトウェア更新)、エディション(ホームの M1 / PRO)、画面のスクリーンショット、運転中かどうか、再現手順。サポート:<b>motorlab.tw@gmail.com</b></p>"
            )},
        ],
    },
}
