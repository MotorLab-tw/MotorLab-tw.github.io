    ...
    "sections": {
        "zh": [
            ...
            {"id": "bearing", "t": "十三、軸承阻力測試", "html": (
                "<p class='manual-intro'>量軸承順暢度,任何版本可用。<b>完全靜止時間越久 → 軸承越順</b>。</p><p>首頁 →「軸承阻力測試」→ 選測試電壓(<b>2.4V 或 3V</b>)→「開始測試」→ 機器加速到該電壓 → 穩定轉速 5 秒 → <b>直接斷電(不剎車)</b> → 計時到完全靜止 → 顯示 <b>完全靜止時間</b>(小數第 2 位)與<b>量測溫度</b>。</p><div class='manual-note'><b>注意</b>:馬達空載慣性小,本測試<b>只顯示時間、不做好壞評定</b>,請拿同一顆馬達不同時期、或不同馬達的時間互相比較;需轉速感應器正常;建議測 2~3 次看重複性。溫度低時油較稠、時間會偏短——系統必要時會<b>自動準備後再測</b>;溫度過低或過高時會提示「數據僅供參考」。</div>"
            )},
        ...
        "ja": [
            ...
            {"id": "bearing", "t": "十三、ベアリング抵抗テスト", "html": (
                "<p class='manual-intro'>ベアリングの滑らかさを測定、どの版でも可。<b>完全停止までの時間が長いほど → ベアリングが滑らか</b>。</p><p>ホーム →「ベアリング抵抗テスト」→ テスト電圧選択(<b>2.4V または 3V</b>)→「テスト開始」→ 機器がその電圧まで加速 → 安定回転 5 秒 → <b>直接電源断(ブレーキなし)</b> → 完全停止まで計時 → <b>完全停止時間</b>(小数第 2 位)と<b>測定温度</b>を表示。</p><div class='manual-note'><b>注意</b>:無負荷慣性が小さいため、本テストは<b>時間のみ表示・良否評定なし</b>、同一個体の時期別や別個体の時間を互いに比較してください;回転センサー正常が必要;再現性は 2~3 回。温度が低いとオイルが粘く時間が短めに出ます——システムは必要に応じて<b>自動準備してから測定</b>します;温度が低すぎるか高すぎる場合は「データは参考のみ」と表示します。</div>"
            )},
