...
FAQ = {
    "zh": [
        ...
        {
            "q": "為什麼即時遙測對馬達磨合很重要?",
            "a": "即時遙測讓磨合過程從「定時跑完就好」變成「依實際狀態收斂」。MotorLab 透過即時電流量測偵測電刷接觸不穩與軸承衰減(τ),並以「電流是否還會再創新低」判定磨合完成,磨到位才自動停止。",
        },
        {
            "q": "什麼是 Gen5 馬達調校系統?",
    ...
    "en": [
        ...
        },
        {
            ...
            "a": "Real-time telemetry turns break-in from 'just run the timer' into 'converge on actual motor state'. MotorLab uses real-time current measurement to detect brush-contact instability and bearing tau decay, judging completion by whether the current can still set a new low and stopping automatically only once it is there.",
        },
        {
    ...
    "ja": [
        ...
        },
        {
            ...
            "a": "リアルタイムテレメトリーにより、慣らしが「タイマーで終わらせる」から「実際のモーター状態に収束させる」へ進化します。MotorLab はリアルタイム電流測定でブラシ接触の不安定とベアリング τ 減衰を検出し、「電流がまだ最低値を更新するか」で完了を判定して、仕上がってはじめて自動停止します。",
        },
        {
            "q": "Gen5 モーター調整システムとは?",
