    ...
    "sections": {
        "zh": [
            ...
            {"id": "standardflow", "t": "七、標準磨合流程(磨前量測 → 磨合 → 磨後量測)", "html": (
                "<p class='manual-intro'>想知道「磨合到底有沒有效、效果多大」,建議照這個三步驟標準流程。</p><p><b>① 磨前量測</b>:先跑一次「<b>AI 扭力預測</b>」(約 1~2 分鐘),記下這顆的<b>內耗％／體質％／扭力</b>——這是磨合前的基準。</p><p><b>② 磨合</b>(兩條路擇一):</p><ul><li><b>AI 智慧磨合</b>:啟動、讓它磨到自動停(「已磨合至最佳狀態」);之後看<b>磨合導引</b>橫幅決定要不要再磨一場(顯示「已到黃金點」就收工)。</li><li><b>自己的配方</b>:用「馬達磨合」(電壓／轉速模式)跑你慣用的配方——磨前磨後量測照樣對照,<b>你的配方到底磨出多少效果,數據會告訴你</b>。</li></ul><p><b>③ 磨後量測</b>:再跑一次「AI 扭力預測」,與磨前對照:</p><table class='manual-table'><thead><tr><th>指標</th><th>磨合有效的樣子</th><th>怎麼解讀</th></tr></thead><tbody><tr><td><b>內耗％</b></td><td><b>明顯下降</b></td><td><b>主判讀指標</b>——內部摩擦與損耗變小＝真的磨開了;降到低點不再降＝已磨透</td></tr><tr><td>扭力(@2.4V／3.0V)</td><td>持平或略升</td><td>扭力主要由磁鐵決定,磨合不會大改;若明顯下降屬異常,建議檢查</td></tr><tr><td>體質％</td><td>通常上升</td><td>綜合分數;<b>前後差在 ±5 以內屬量測散布</b>,別過度解讀單次數字</td></tr><tr><td>磨合前／後轉速(磨合頁顯示)</td><td>上升</td><td>同電壓下轉速提升＝接觸改善的最直接體現</td></tr></tbody></table><div class='manual-note'><b>全新品註</b>:全新未磨的馬達,內耗％的<b>起點高低因個體與油況而異</b>——實測有磨合後<b>明顯下降</b>的,也有本來就乾淨低阻、磨後<b>持平甚至微升</b>的(電刷貼合面積變大、初生碳粉),<b>兩種都正常</b>。所以全新品<b>不要用內耗％單獨判讀磨合效果</b>,請主要看<b>磨合指標曲線下降</b>與<b>磨合前後轉速上升</b>;「內耗明顯下降＝磨開了」的判讀主要適用於<b>用過、變髒或久置的馬達</b>。</div><p><b>④(建議)建立健康指紋</b>:磨合完成＝這顆的巔峰狀態——此時到「AI 智慧馬達健康管理」建立<b>健康指紋</b>,之後定期檢測就能掌握它離巔峰多遠(詳見「十二、AI 智慧馬達健康管理」)。</p><div class='manual-note'><b>量測時機提醒(重要)</b>:磨合結束後馬達是熱的,<b>熱態量測數值會偏保守</b>(磁力隨溫度略降、油與碳粉尚未沉澱),直接量會低估磨合效果。正確做法:磨前量測時記下畫面顯示的「溫度」;磨合完成後<b>等溫度降回與磨前相同(±0.5°C 內)再做磨後量測</b>——同溫對照,前後數據才真正可比。</div>"
            )},
        ...
        "en": [
            ...
            {"id": "standardflow", "t": "7. Standard Break-in Flow (Before → Break-in → After)", "html": (
                "<p class='manual-intro'>To know &quot;whether break-in actually worked, and by how much,&quot; follow this three-step standard flow.</p><p><b>① Measure before</b>: first run <b>AI Torque Prediction</b> (about 1–2 min) and note this motor's <b>internal loss % / constitution % / torque</b> — the pre-break-in baseline.</p><p><b>② Break in</b> (pick one path):</p><ul><li><b>AI Smart Break-in</b>: start it and let it stop itself (&quot;broken in to best state&quot;); then read the <b>Break-in Guide</b> banner to decide on another session (&quot;sweet spot reached&quot; = done).</li><li><b>Your own recipe</b>: use &quot;Break-in&quot; (voltage / speed mode) with your usual recipe — before/after measurements still compare, so <b>the data tells you exactly how much your recipe achieved</b>.</li></ul><p><b>③ Measure after</b>: run AI Torque Prediction again and compare with before:</p><table class='manual-table'><thead><tr><th>Metric</th><th>Effective break-in looks like</th><th>How to read</th></tr></thead><tbody><tr><td><b>Internal loss %</b></td><td><b>Clear drop</b></td><td><b>The primary metric</b> — less internal friction/loss = genuinely opened up; dropped to a low and no longer falling = fully broken in</td></tr><tr><td>Torque (@2.4V / 3.0V)</td><td>Flat or slightly up</td><td>Torque is set mainly by the magnet; break-in won't change it much; a clear drop is abnormal — inspect</td></tr><tr><td>Constitution %</td><td>Usually up</td><td>A composite score; <b>a before/after gap within ±5 is measurement spread</b> — don't over-read a single number</td></tr><tr><td>Pre/post-break-in RPM (break-in page)</td><td>Up</td><td>Higher RPM at the same voltage = the most direct sign of improved contact</td></tr></tbody></table><div class='manual-note'><b>Brand-new note</b>: for a brand-new un-run motor the <b>internal loss % starting point varies by unit and oil condition</b> — in practice some <b>drop clearly</b> after break-in, while others are already clean and low-resistance and <b>stay flat or even rise slightly</b> (larger brush contact area, fresh carbon); <b>both are normal</b>. So for brand-new motors <b>don't read the break-in effect from internal-loss % alone</b> — mainly watch the <b>break-in metric curve dropping</b> and <b>pre/post RPM rising</b>. The &quot;clear internal-loss drop = bedded in&quot; reading applies mainly to <b>used, dirtied or long-stored motors</b>.</div><p><b>④ (Recommended) Build a health fingerprint</b>: break-in complete = this motor's peak state — now open &quot;AI Motor Health Manager&quot; and build a <b>health fingerprint</b>, so periodic checks later show how far it has drifted from peak (see &quot;12. AI Motor Health Manager&quot;).</p><div class='manual-note'><b>Timing reminder (important)</b>: the motor is hot right after break-in, and <b>hot-state readings run conservative</b> (magnetism dips slightly with temperature; oil and carbon haven't settled), so measuring immediately underestimates the effect. Correct approach: note the on-screen &quot;temperature&quot; at the before measurement; after break-in, <b>wait for the temperature to return to the same as before (within ±0.5°C) before the after measurement</b> — same-temperature comparison makes before/after truly comparable.</div>"
            )},
        ...
        "ja": [
            ...
            {"id": "standardflow", "t": "七、標準慣らしフロー(慣らし前測定 → 慣らし → 慣らし後測定)", "html": (
                "<p class='manual-intro'>「慣らしが本当に効いたか、どれだけ効いたか」を知りたいなら、この 3 ステップの標準フローを推奨します。</p><p><b>① 慣らし前測定</b>:まず「<b>AI トルク予測</b>」を 1 回(約 1~2 分)実行し、この個体の<b>内耗％／素性％／トルク</b>を記録——これが慣らし前の基準。</p><p><b>② 慣らし</b>(2 つの道から一つ):</p><ul><li><b>AI スマート慣らし</b>:開始して自動停止まで任せる(「最適状態まで慣らし完了」);その後<b>慣らしガイド</b>のバナーでもう 1 回やるか判断(「ゴールデンポイント到達」で完了)。</li><li><b>自分の配合</b>:「モーター慣らし」(電圧／回転数モード)で慣れた配合を実行——前後測定は同じく対照でき、<b>あなたの配合がどれだけ効果を出したか、データが教えます</b>。</li></ul><p><b>③ 慣らし後測定</b>:再度「AI トルク予測」を実行し、前と対照:</p><table class='manual-table'><thead><tr><th>指標</th><th>慣らしが効いた様子</th><th>解釈</th></tr></thead><tbody><tr><td><b>内耗％</b></td><td><b>明確に低下</b></td><td><b>主判読指標</b>——内部摩擦と損失が減った＝本当に開いた;下がりきってこれ以上下がらない＝慣れ切った</td></tr><tr><td>トルク(@2.4V／3.0V)</td><td>横ばいまたは微増</td><td>トルクは主に磁石で決まり慣らしで大きく変わらない;明確に低下は異常、点検を</td></tr><tr><td>素性％</td><td>通常は上昇</td><td>総合スコア;<b>前後差 ±5 以内は測定ばらつき</b>、単発の数字を深読みしない</td></tr><tr><td>慣らし前／後の回転数(慣らしページ表示)</td><td>上昇</td><td>同電圧で回転数が上がる＝接触改善の最も直接的な現れ</td></tr></tbody></table><div class='manual-note'><b>新品の注記</b>:新品未慣らしのモーターは、内耗％の<b>起点が個体とオイル状況により異なります</b>——実測では慣らし後に<b>明確に低下</b>する個体もあれば、元来クリーンで低抵抗のため<b>横ばい、あるいは微増</b>する個体もあります(ブラシ接触面積の増加、初生カーボン)、<b>どちらも正常</b>です。したがって新品では<b>内耗％だけで慣らし効果を判読しない</b>でください——主に<b>慣らし指標曲線の低下</b>と<b>慣らし前後の回転数上昇</b>を見てください。「内耗が明確に低下＝慣れた」の判読は主に<b>使用済み・汚れた・長期保管のモーター</b>に当てはまります。</div><p><b>④(推奨)健康指紋を作成</b>:慣らし完了＝この個体のピーク状態——この時「AI スマートモーター健康管理」で<b>健康指紋</b>を作れば、以後の定期検査でピークからどれだけ離れたか把握できます(「十二、AI スマートモーター健康管理」参照)。</p><div class='manual-note'><b>測定タイミングの注意(重要)</b>:慣らし直後のモーターは熱く、<b>熱状態の測定値は保守寄り</b>(磁力が温度で少し下がり、オイルとカーボンが未沈殿)で、すぐ測ると効果を過小評価します。正しい方法:慣らし前測定時に画面表示の「温度」を記録;慣らし完了後は<b>温度が慣らし前と同じ(±0.5°C 以内)に戻ってから慣らし後測定</b>——同温対照で前後データが真に比較可能になります。</div>"
            )},
