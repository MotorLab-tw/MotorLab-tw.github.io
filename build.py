    ...
    "sections": {
        "zh": [
            ...
            {"id": "settings", "t": "十三、系統設定", "html": (
                "<p class='manual-intro'>系統運轉中此頁只留「回首頁」可按,其餘鎖定。</p><table class='manual-table'><tbody><tr><th>使用者設定</th><td>名稱 / 國家(32 字以內,預設 <code>--</code>),寫入每筆紀錄當出處。已產生的紀錄不回填。</td></tr><tr><th>WiFi 設定</th><td>改熱點名稱 / 密碼(8~63 字)→ 儲存後機器重啟,須重新連新熱點。忘密碼見第十五章。</td></tr><tr><th>語言</th><td>中文 / English / 日本語 切換(預設中文)。按「確定」後<b>系統重新啟動</b>乾淨載入(約 10 秒);運轉中切換則只重載頁面、不中斷程序。</td></tr><tr><th>正版查驗</th><td>顯示本機 ID,可至官網查驗頁自行確認機器為正版(v3.4.1 起)。</td></tr><tr><th>溫度校正</th><td>顯示與實際有偏差時加補償(±20°C)。</td></tr><tr><th>高溫鎖定</th><td>設定高溫鎖定溫度(預設 50°C,範圍 25~60°C)。當前溫度高於設定值 5°C 以上持續 10 秒即進入高溫鎖定(停機,須按歸零解除)。區塊內同時顯示當前溫度。</td></tr><tr><th>平均轉速設定</th><td>容差(相鄰兩秒轉速差)下拉 120~600 RPM,<b>預設 240</b>。容差越大越容易判定穩定但反應較遲鈍;馬達較不穩可調高。</td></tr><tr><th>緩啟動</th><td>電壓由 0 在 5 秒內平順線性升到目標電壓(與緩停止同風格),<b>無可調參數</b>。電壓輸出已校準,不需設定啟動扭矩。</td></tr><tr><th>取得授權</th><td>M1 升 Pro,見下。</td></tr><tr><th>軟體更新</th><td>透過家裡 WiFi 自動取得最新韌體,見下。</td></tr><tr><th>RGB 狀態燈</th><td>雙燈(內建 / 面板)輸出位置 + 亮度(0~100%)+ 5 組狀態樣式,見下。</td></tr><tr><th>工程模式</th><td>密碼保護的進階校正與離線更新(一般使用者用不到)。</td></tr><tr><th>WiFi 列表</th><td>最多記憶 8 組外部 WiFi;「清空列表」用於轉手設備(不影響 Pro 授權)。</td></tr></tbody></table><p><b>取得授權(M1 → Pro)</b>:系統設定 →「取得授權」→ 選家裡 WiFi 連線 → 機器自動驗證購買 → 未購買則顯示 QR Code → 用另一台有外網的裝置掃碼結帳 → 機器每 10 秒自動查詢,付款成功即解鎖。全程約 1~3 分鐘、<b>不必手動輸入金鑰</b>、最久 10 分鐘逾時可重試。</p><p><b>軟體更新</b>:系統設定 →「檢查更新」→ 選 WiFi → 有新版自動下載寫入(進度到 100%)→ 自動重啟、頁面自動重載。<b>過程勿關電源 / 瀏覽器</b>(中途斷電有保護自動回舊版);下載約 1.6MB,家用 WiFi 約 5~10 秒。</p><p><b>RGB 狀態燈</b>:輸出位置(面板 / 面板+內建 / 內建,即時切換)、亮度(拖滑桿 →「預覽」試 5 秒 → 滿意「套用」儲存)、狀態樣式 5 組(狀態=無 / 高溫 / 冷卻 / 待機 / 運轉 × 色相 × 模式 × 週期,同樣預覽 / 套用兩步)。</p><p><b>工程模式</b>:需密碼(密碼錯 3 次鎖 60 秒,10 分鐘無操作自動登出)。</p>"
            )},
            {"id": "safety", "t": "十四、安全與保護", "html": (
                "<p class='manual-intro'>機器在任何驅動馬達的程序(磨合 / 測試 / 特性量測 / 軸承 / 電刷 / 健康檢測)中全程自動監測,異常時<b>立即停止並跳出說明框</b>:</p><table class='manual-table'><thead><tr><th>保護</th><th>觸發時機</th><th>反應</th></tr></thead><tbody><tr><td>未偵測到馬達</td><td>通電但無馬達電流(未裝或接線鬆脫)</td><td>約 3 秒內停</td></tr><tr><td>馬達卡住</td><td>高電流但不轉(卡死)</td><td>約 5 秒內停</td></tr><tr><td>讀不到轉速訊號</td><td>有吃電流但轉速持續為 0(<b>馬達未上治具</b>或感測異常)</td><td>約 4 秒內停</td></tr><tr><td>輸出短路</td><td>輸出端子近乎短路</td><td>緊急停止(0.15 秒)</td></tr><tr><td>高溫</td><td>超過設定溫度</td><td>立即停 + 鎖定</td></tr></tbody></table><p>觸發時<b>蜂鳴器響 4 長聲</b>(與高溫 5 短聲區別),畫面彈出「運轉已停止」+ 原因,排除後可重新啟動。<b>供電規格 DC 5V / 6A(不可低於)</b>;供電不足時頂部會出現琥珀色警示「可能無法達到設定電壓」——機器採即時電壓補償,供電夠力才能輸出準確電壓。</p><table class='manual-table'><thead><tr><th>機制</th><th>觸發 / 行為</th></tr></thead><tbody><tr><td>高溫保護</td><td>兩種觸發:① 溫度感測器警報;② 當前溫度高於「高溫鎖定溫度」設定值 5°C 以上持續 10 秒。任一觸發 → 立即停 + 嗶 5 聲 + 鎖定。<b>即使溫度已降回也須按「歸零」才解除</b>(仍高溫時按歸零會再次鎖定)。</td></tr><tr><td>緩啟動 / 緩停止</td><td>啟動為 5 秒線性升壓(0V→目標電壓),停止為 3 秒線性下降,避免電流爆衝與機械衝擊。</td></tr><tr><td>電流上限</td><td>固定 4A 量測上限,超過讀值封頂(不停機,但長時間高電流可能觸發高溫)。</td></tr><tr><td>開機鎖定</td><td>開機 30 秒內連續 3 次崩潰 → 自動切回上一版韌體。</td></tr></tbody></table>"
            )},
        ...
        "ja": [
            ...
            {"id": "safety", "t": "十四、安全と保護", "html": (
                "<p class='manual-intro'>モーターを駆動するあらゆる工程(慣らし / テスト / 特性測定 / ベアリング / ブラシ / 健康検査)で機器は全自動で監視し、異常時は<b>直ちに停止して説明を表示</b>します:</p><table class='manual-table'><thead><tr><th>保護</th><th>トリガー</th><th>反応</th></tr></thead><tbody><tr><td>モーター未検出</td><td>通電しているがモーター電流なし(未装着 / 配線緩み)</td><td>約 3 秒以内に停止</td></tr><tr><td>モーター拘束</td><td>高電流だが回らない(固着)</td><td>約 5 秒以内に停止</td></tr><tr><td>回転信号なし</td><td>電流は流れるが回転が 0 のまま(<b>治具未装着</b>やセンサー異常)</td><td>約 4 秒以内に停止</td></tr><tr><td>出力短絡</td><td>出力端子がほぼ短絡</td><td>緊急停止(0.15 秒)</td></tr><tr><td>高温</td><td>設定温度を超過</td><td>直ちに停止 + ロック</td></tr></tbody></table><p>トリガー時は<b>ブザーが長音 4 回</b>(高温の短音 5 回と区別)、画面に「運転を停止しました」+ 理由を表示、解消後に再起動可。<b>電源仕様 DC 5V / 6A(下回らないこと)</b>;供給不足時は上部に琥珀色の警告が出ます——機器はリアルタイム電圧補償を行うため、正確な出力には十分な供給が必要です。</p><table class='manual-table'><thead><tr><th>機構</th><th>トリガー / 挙動</th></tr></thead><tbody><tr><td>高温保護</td><td>2 つのトリガー:① 温度センサー警報;② 現在温度が「高温ロック温度」設定より 5°C 以上高い状態が 10 秒。いずれかで → 直ちに停止 + ビープ 5 回 + ロック。<b>温度が下がっても「ゼロリセット」を押すまで解除されません</b>(高温のまま押すと再ロック)。</td></tr><tr><td>ソフトスタート / ストップ</td><td>起動は 5 秒の線形昇圧(0V→目標)、停止は 3 秒の線形降圧で、電流の急増と機械的衝撃を回避。</td></tr><tr><td>電流上限</td><td>4A 固定の測定上限、超過分は読値を頭打ち(停止しないが、長時間の高電流は高温を誘発しうる)。</td></tr><tr><td>起動ロック</td><td>起動 30 秒以内に 3 回連続クラッシュ → 前バージョンのファームへ自動復帰。</td></tr></tbody></table>"
            )},
            ...
            {"id": "faq", "t": "十六、よくある質問", "html": (
                "<table class='manual-table'><thead><tr><th>症状</th><th>対処</th></tr></thead><tbody><tr><td>起動音が鳴らない</td><td>電源 5V とブザー配線を確認</td></tr><tr><td>接続できるがページが開けない</td><td><code>http://</code>(https でない)を確認、モバイルデータをオフ、強制再読み込み(Ctrl+Shift+R)</td></tr><tr><td>「適用中…」のまま</td><td>多くは WiFi 不安定、ページを再読み込み(内蔵の 8 秒監視が赤字で知らせます)</td></tr><tr><td>WiFi パスワードを忘れて接続できない</td><td>機器の WiFi リセット接点を 5 秒長押し(「十五、WiFi リセット」参照)、ホットスポット名とパスワードのみリセットされ、他の設定と記録はすべて保持</td></tr><tr><td>長音 4 回で自動停止、復帰するには</td><td>画面表示の原因を解消(モーター未固定 / 端子接触不良 / 出力短絡 / センサー未整合)→ 再起動不要、そのまま再度「開始」で故障状態は自動クリア</td></tr><tr><td>起動して数秒で自動停止する</td><td>通常は保護動作、画面の原因を確認:電流が低すぎ=モーター未接続;電流が高すぎ=固着または短絡;供給不足の赤い警告がないかも確認</td></tr><tr><td>モーターは回るが回転数が 0 表示</td><td>反射マークがセンサーに向いているか、治具が整合しているか確認;センサー部への強い光を避ける。読めない状態が続くと保護で自動停止(約 4 秒)</td></tr><tr><td>高温ロックが解除できない</td><td>温度が閾値未満に下がってから「ゼロリセット」;室温が高い場合はシステム設定で閾値を上げる(上限 60°C)か温度校正の補償を確認</td></tr><tr><td>慣らし途中でモーター交換</td><td>「停止」→ 交換 → 再度「開始」(新規は別の記録)</td></tr><tr><td>回転数モードで段階が「未達」</td><td>目標回転数がモーター能力を超過(安全上限電圧で時間いっぱい運転して終了)→ 目標を下げる;明らかに力があるなら治具の取付とセンサーを確認</td></tr><tr><td>スマートモードの対応機種</td><td>カーボンブラシ Dash 系(ドロップダウンの 5 機種);他機種は電圧または回転数モード</td></tr><tr><td>スマートモードが時々長く回るのはなぜ</td><td>「定常状態を検出」するまで止まらず、モーター差が大きいため;時間上限(2h / 3h / 4h)が兜底</td></tr><tr><td>慣らしは何パスで十分か</td><td>決まった答えはなく、「安定電流」指標が収束するかを見る;自分で判断したくなければスマートモードで定常状態まで自動停止</td></tr><tr><td>「導電慣らしオイル」を入れると数値が特に良い</td><td>それは炭粉が一時的に導通する<b>錯覚</b>で、真の慣らしではありません。オイルを洗浄してから測定を;本機はモーターの真の電気状態で判定し、導電オイルは読値を楽観側に歪めます(「五、モーター慣らし」の警告参照)</td></tr><tr><td>同じモーターでも毎回数値が少し違う</td><td>正常です。ブラシ接触と温度で小幅な自然ドリフトが生じます;判読は<b>トレンドと複数回平均</b>で、単発を比べない</td></tr><tr><td>自分のデータが他人 / ネットと合わない</td><td>電圧基準、供給、治具、測定方法の違いが系統的な差を生みます;<b>同一機器での前後比較</b>が最も信頼できます</td></tr><tr><td>KV 値とは</td><td>現在回転数 ÷ モーター実効電圧(rpm/V)、モーター状態を素早く比べる指標 —— 同電圧で数値が高いほど速い</td></tr><tr><td>対応するモーターは</td><td>タミヤ Mini 4WD 130 型カーボンブラシモーター(片軸 / 両軸とも治具に装着可);ブラシレスは非対応</td></tr><tr><td>電源の要件は</td><td>DC 5V / 6A 以上;供給不足時は UI に警告(待機電圧が低い、負荷で垂下)、細いケーブルや小容量アダプターで起こりがち</td></tr><tr><td>ライセンス取得が接続で止まる</td><td>家庭 WiFi のパスワード誤り / ネットなし / 電波弱 → 再入力または WiFi 変更</td></tr><tr><td>Pro 返金後</td><td>次のオンライン再検証時に<b>自動で M1 へ戻る</b>;再購入は「ライセンス再取得」</td></tr><tr><td>赤い「AP パスワードが初期のまま」バナー</td><td>WiFi 設定でパスワードを変更</td></tr><tr><td>他人に譲る</td><td>エンジニアリングモードで校正リセット → WiFi リスト消去 →(任意)記録消去 / モーター削除 → WiFi リセット。<b>Pro ライセンスは機器紐付けで譲渡不可</b></td></tr><tr><td>測定データはクラウドにアップロードされる?</td><td>されません。100% 機器内に保存;自分で「アップロード」したレシピのみグローバルデータ庫へ</td></tr><tr><td>履歴が 50 件に達したら</td><td>初期設定では最古を自動上書き(無効化可);重要な記録は先に JSON/CSV でエクスポートを</td></tr><tr><td>ソフトウェア更新で記録は消える?</td><td>消えません。更新は全設定・校正・履歴を保持</td></tr><tr><td>更新失敗で文鎮化する?</td><td>しません。前バージョンへの自動復帰があります</td></tr><tr><td>待機中に電源を抜いてよい?</td><td>可。モーター完全停止・「適用中」なし・更新中でないときが安全</td></tr></tbody></table>"
            )},
