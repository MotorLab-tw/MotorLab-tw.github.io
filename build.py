    ...
    "sections": {
        "zh": [
            ...
            {"id": "smartbreakin", "t": "六、AI 智慧磨合(beta)(Pro)", "html": (
                "<p class='manual-intro'>一鍵全自動磨合:系統自己磨、自己量,<b>一場磨到這顆的最佳狀態、自動停止</b>——不必分多場、不必自己判斷要磨幾次。</p><p><b>操作</b>:首頁 →「AI 智慧磨合(beta)」→ 選<b>馬達型號</b>(田宮全 15 款)＋填<b>備註</b>(選填,40 字以內)＋選<b>時間上限</b>(磨到完成為止／2h／3h／4h)→「啟動」,其餘全自動——系統以漸進轉速磨合並持續量測,判定<b>已磨至最佳狀態</b>時自動停止並嗶聲提示。</p><ul><li>頁面上方的即時數據與圖表<b>與磨合頁完全相同</b>;下方顯示:目前輪次／目前動作／已進行時間／收斂進度,及主視覺落點圖「<b>運轉穩定度</b>」。</li><li>過程中馬達會<b>正反轉交替</b>磨合(電刷兩面對稱貼合),並週期性自動量測,聲音與轉速有規律變化屬正常。</li><li><b>自動判定取樣</b>:第一輪為<b>熱身輪</b>(磨合照常進行,不列入數據與曲線);量測前系統會自動準備(必要時馬達暫停轉動屬正常),使各輪判定樣本條件一致。</li><li>按「停止」<b>立即停止</b>,已完成輪次的數據照常保留在畫面上。</li></ul><h3>運轉穩定度——一眼看懂磨合進展</h3><p><b>運轉穩定度(%)</b>是馬達運轉中,電刷與整流子接觸品質的評分(0~100)。全新馬達的接觸面尚未貼合,運轉時接觸忽好忽壞,分數偏低;隨磨合進行,電刷與整流子互相磨出貼合面,接觸越來越安定,分數逐步爬升,最後停在這顆馬達自己的穩定帶——這就是「磨順了」。</p><table class='manual-table'><thead><tr><th>曲線長相</th><th>代表什麼</th></tr></thead><tbody><tr><td><b>由低爬高、然後走平</b></td><td>標準磨合歷程——正在磨開,最後到位</td></tr><tr><td><b>開場就高、全程平穩</b></td><td>這顆先前已磨合到位</td></tr><tr><td><b>磨了許久仍偏低或大幅跳動</b></td><td>接觸不良的徵兆(髒污、氧化、電刷／整流子耗損);系統判定磨不出改善時會如實告知</td></tr></tbody></table><p>縱軸固定 0~100,跨場可直接比較;分數以<b>同一顆馬達自我比較</b>最準,不同型號各有天生的穩定帶。點圖上任一落點可看該輪數值。</p><h3>完成訊息</h3><p><b>完成時</b>顯示磨前後對照(顯示磨合前／後轉速與提升幅度),結果訊息如實告知:</p><table class='manual-table'><thead><tr><th>完成訊息</th><th>意思</th></tr></thead><tbody><tr><td>✓ 磨合完成——這顆馬達已磨順,可以上場</td><td>正常完成,這顆收工</td></tr><tr><td>⚠ 此馬達磨無改善且訊號劣化——判定為嚴重耗損,不建議繼續磨合</td><td>系統判定這顆已嚴重耗損、磨不出改善——如實告知,別再花時間</td></tr></tbody></table><div class='manual-note'><b>注意</b>:曲線與數據<b>只顯示本場</b>(不留歷史紀錄);按「歸零」或啟動新場即清除。想留存結果,完成畫面的磨前後對照(磨合前／後轉速與提升幅度)就是這場的成績單。對<b>已磨到位的馬達</b>再啟動:系統會快速確認狀態後自動停止(顯示磨合完成)——不會傷馬達,但也沒必要常做。</div>"
            )},
            ...
            {"id": "health", "t": "十二、AI 智慧馬達健康管理(Pro)", "html": (
                "<p class='manual-intro'>為每顆馬達建立<b>健康指紋</b>——磨合完成時的巔峰狀態快照——之後定期重測,直接看<b>這次與上次的差異</b>。<b>跟自己比,不跟別顆比</b>。<br><b>量測誤差:&#177;5%</b></p><p><b>正確使用順序(重要)</b>:先把馬達<b>磨合完成</b>(建議用 AI 智慧磨合磨到自動停),再建立指紋。指紋代表這顆馬達的最佳狀態,之後每次檢測都跟它與上一次比。</p><ol><li>首頁 →「AI 智慧馬達健康管理」→「+ 新增馬達」。</li><li>填 <b>馬達型號</b>(下拉,必選——診斷需要對照該型號的標準)+ <b>備註</b>(選填)。</li><li>確認 → 自動跑約 <b>7~8 分鐘</b> 建立指紋(特性量測 3 趟 → 電刷接觸 → 換向紋波紀錄;含各段自動準備)。若機器提示「<b>可能尚未磨透</b>」:代表這顆的內部損耗還偏高,建議先跑 AI 智慧磨合,完成後重建指紋。</li></ol><p>之後每張卡片可做:</p><ul><li><b>完整檢測</b>:<b>磁力／內部損耗／電刷接觸</b>三個面向對指紋比對,最完整。</li><li><b>快速檢測</b>:只比對內部損耗,適合日常快篩。</li></ul><p>結果頁直接列出<b>這次與上次的差異</b>——磁力／內部損耗／電刷穩定度三行,每行給「<b>上次｜這次｜變化</b>」,並附這次的量測溫度。</p><div class='manual-note'>機器告訴你變化了多少,怎麼判讀由你決定。磁力數值已做溫度校正,跨季比較較為可靠;內部損耗與電刷穩定度未校正,請盡量在相近溫度下比較。<b>±2% 以內的變化多半是量測散布,不是馬達真的變了。</b></div><p>馬達卡片上顯示最近一次檢測時間(只建過指紋、還沒檢測過的顯示「已建指紋 + 時間」)。詳情頁可看歷次檢測的三個量測值,以及<b>量測變化趨勢圖</b>(三個量各自對視窗內最舊一筆歸一為 100%)。</p><p><b>什麼時候要重建指紋</b>:洗馬達／深度保養之後、或檢測顯示「比指紋更順了」時——把新狀態設為新基準。舊指紋會保留在歷史裡,成為這顆馬達的生命履歷。</p><div class='manual-note'><b>注意</b>:每顆最多 50 次歷史,整機最多 20 顆;檢測中勿斷電／勿按其他鈕;高溫會自動中止;舊版建立的指紋需重建一次;M1 版按鈕灰色(點下提示升級)。</div>"
            )},
